...
def plotModel(model, df):
    ...
    gridxyz = numpy.column_stack(
        (numpy.tile(gridx, len(gridz)), numpy.zeros(len(gridx) * len(gridz)), numpy.repeat(gridz, len(gridx)))
    )
    ...
    ax = (
        numpy.array(
            ...
            + [plt.axes([0.425, 0.81 - 0.195 * i, 0.38, 0.18]) for i in range(5)]
        )
        .reshape(2, 5)
        .T
    )
    ax[0, 0].contour(
        ...
        gridz,
        numpy.log10(rho_nsd),
        levels=numpy.log10(numpy.max(rho_nsd)) + numpy.linspace(-6, -0, 16),
        cmap="Blues",
    )
    ax[0, 1].contour(
        gridx,
        gridz,
        numpy.log10(Sig_nsd),
        levels=numpy.log10(numpy.max(Sig_nsd)) + numpy.linspace(-6, -0, 16),
        cmap="Blues",
    )
    ...
    plt.clabel(
        ax[1, 0].contour(gridx, gridz, -vel[:, 2].reshape(len(gridz), len(gridx)), levels=gridv, cmap="Blues"),
        fmt="%.0f",
    )
    plt.clabel(
        ax[2, 0].contour(
            gridx,
            gridz,
            numpy.sqrt(vel2[:, 2] - vel[:, 2] ** 2).reshape(len(gridz), len(gridx)),
            levels=gridv,
            cmap="Blues",
        ),
        fmt="%.0f",
    ...
    plt.clabel(
        ax[3, 0].contour(
            gridx,
            gridz,
            numpy.sqrt(vel2[:, 0] - vel[:, 0] ** 2).reshape(len(gridz), len(gridx)),
            levels=gridv,
            cmap="Blues",
        ),
        fmt="%.0f",
    ...
    plt.clabel(
        ax[4, 0].contour(
            gridx,
            gridz,
            numpy.sqrt(vel2[:, 1] - vel[:, 1] ** 2).reshape(len(gridz), len(gridx)),
            levels=gridv,
            cmap="Blues",
        ),
        fmt="%.0f",
    ...
    plt.clabel(
        ax[1, 1].contour(gridx, gridz, -vel[:, 2].reshape(len(gridz), len(gridx)), levels=gridv, cmap="Blues"),
        fmt="%.0f",
    )
    plt.clabel(
        ax[2, 1].contour(
    ...
    for i in range(5):
        ...
        ax[i, 0].text(
            0.5, 0.98, "intrinsic " + labels[i], ha="center", va="top", transform=ax[i, 0].transAxes, color=colors[i]
        )
        ax[i, 1].text(
            0.5, 0.98, "projected " + labels[i], ha="center", va="top", transform=ax[i, 1].transAxes, color=colors[i]
        )
        if i < 4:
    ...
    for i in range(4):
        ...
        if i == 0:
            ...
            ax.text(
                1.0, 1.05, "projected velocity distributions in fields", ha="right", va="center", transform=ax.transAxes
            )
...
if __name__ == "__main__":
    ...
    sigmamin = 2.0
    Jmin = 10.0
    dfNSD = agama.DistributionFunction(
