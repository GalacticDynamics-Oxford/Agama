...
try:
    from time import process_time as clock  # Python 3.3+
except ImportError:
    from time import clock  # older Python
...
try:
    ...
    exit("Input snapshot files are not available; you may create them by running example_self_consistent_model.py")

print(
    "%g s to load %d disk particles (total mass=%g Msun) "
    "and %d halo particles (total mass=%g Msun)"
    % (
        clock() - tbegin,
        diskParticles.shape[0],
        numpy.sum(diskParticles[:, 6]),
        haloParticles.shape[0],
        numpy.sum(haloParticles[:, 6]),
    )
)
...
try:
    ...
    haloPot = agama.Potential(
        type="Multipole", particles=(haloParticles[:, 0:3], haloParticles[:, 6]), symmetry="a", gridsizeR=20, lmax=2
    )
    print(
        "%f s to init %s potential for the halo; value at origin=%f (km/s)^2"
    ...
    diskPot = agama.Potential(
        type="CylSpline",
        particles=(diskParticles[:, 0:3], diskParticles[:, 6]),
        gridsizer=20,
        gridsizez=20,
        symmetry="a",
        Rmin=0.2,
    ...
    print(
        "%f s to init %s potential for the disk; value at origin=%f (km/s)^2"
...
# 5. write out data
Rz = numpy.vstack((numpy.sqrt(diskParticles[:, 0] ** 2 + diskParticles[:, 1] ** 2), diskParticles[:, 2])).T
energy = (totalPot.potential(diskParticles[:, 0:3]) + 0.5 * numpy.sum(diskParticles[:, 3:6] ** 2, axis=1)).reshape(
    -1, 1
)
numpy.savetxt(
    "disk_actions.txt",
    numpy.hstack((Rz, actions, energy)),
