...
class ModelSearcher:
    ...
    def __init__(self):
        ...
        if phase_space_info_mode != 6 or vel_error != 0:
            self.samples, self.weights = sampleMissingData(
                numpy.hstack((self.particles, numpy.ones((self.particles.shape[0], 3)) * vel_error)), num_subsamples
            )
        else:
            self.samples = None
    ...
    def monteCarloSearch(self):
        ...
        if self.values.ndim == 1:
            ...
            for i in range(self.values.shape[0]):
                ...
                if not numpy.isfinite(prob[i, 0]):
                    ...
                    print("%i-th walker: logL=%g" % (i, prob[i, 0]))
        ...
        while True:  # run several passes until convergence
            ...
            numpy.savetxt(
                self.filename + ".best",
                numpy.hstack((self.values, sampler.lnprobability[:, -1].reshape(-1, 1))),
                fmt="%.8g",
            )
            numpy.savetxt(
                self.filename + ".chain",
                numpy.hstack(
                    (
                        sampler.chain[:, -nsteps_mcmc:].reshape(-1, nparams),
                        sampler.lnprobability[:, -nsteps_mcmc:].reshape(-1, 1),
                    )
                ),
                fmt="%.8g",
            )
            ...
            try:
                ...
                pass  # sometimes it can't be computed, then ignore
            ...
            for i in range(nparams):
                ...
                print(
                    "Parameter %20s  avg= %8.5g;  one-sigma range = (%8.5f, %8.5f)"
                    % (
                        self.model.labels[i],
                        avgparams[i],
            ...
            if not prevmaxloglike is None:
                if (
                    maxloglike - prevmaxloglike < 1.0
                    and abs(avgloglike - prevavgloglike) < 1.0
                    and numpy.all(avgparams - prevavgparams < 0.1)
                    and numpy.all(rmsparams - prevrmsparams < 0.1)
                ):
                    print("Converged")
                    return
    ...
    def plot(self, chain, loglike, labels):
        ...
        maxexpected = (
            numpy.median(loglike[:, -nsteps_mcmc:]) + 0.5 * ndim - 0.33
        )  # expected max-likelihood for a chi2 distribution with ndim
        ...
        try:
            ...
            ax.plot(
                xx + maxexpected,
                1 / scipy.special.gamma(0.5 * ndim) * (-xx) ** (0.5 * ndim - 1) * numpy.exp(xx),
                "r",
                lw=2,
            )
