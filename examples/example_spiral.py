...
def createSpiralPotential(numberOfArms, surfaceDensity, scaleRadius, scaleHeight, pitchAngle, phi0=0):
    ...
    def potfnc(xyz):
        ...
        for n in range(1, 4):
            ...
            C_n = [8.0 / 3 / numpy.pi, 0.5, 8.0 / 15 / numpy.pi][n - 1]  # amplitudes of the three harmonic terms
            Phi += prefac * (C_n / D_n / K_n * numpy.cos(n * gamma) * (numpy.cosh(K_n * z / B_n)) ** -B_n)
...
pot_disk = agama.Potential(
    type="disk", surfaceDensity=surfaceDensityD, scaleRadius=scaleRadius, scaleHeight=-scaleHeight
)  # minus sign means the isothermal (sech^2) vertical profile
...
for i in range(nsteps):
    ...
    XYZrot = numpy.column_stack(
        (XYZ[:, 0] * cosa[i] + XYZ[:, 1] * sina[i], XYZ[:, 1] * cosa[i] - XYZ[:, 0] * sina[i], XYZ[:, 2])
    )
