...
def createNewExponentialDF(**params):
    ...
    Jr0, Jz0, Jphi0, pr, pz, addJden, addJvel = (
        float(params[name]) for name in ("Jr0", "Jz0", "Jphi0", "pr", "pz", "addJden", "addJvel")
    )
    norm = 1.0
    norm = float(params["mass"]) / agama.DistributionFunction(df).totalMass()
    print("Created a %s DF with mass %s" % (params["type"], params["mass"]))
...
def writeRadialProfile(model):
    ...
    numpy.savetxt(
        "mwmodel_surface_density.txt",
        numpy.column_stack((radii, Sigma * 1e-6)),
        fmt="%.6g",
        delimiter="\t",
        header="radius\t" + header + "[Msun/pc^2]",
    )
    numpy.savetxt(
        "mwmodel_volume_density.txt",
        numpy.column_stack((radii, z0dens * 1e-9)),
        fmt="%.6g",
        delimiter="\t",
        header="radius\t" + header + "[Msun/pc^3]",
    )
    numpy.savetxt(
        "mwmodel_sigmaR.txt",
        numpy.column_stack((radii, meanv2[:, :, 0] ** 0.5)),
        fmt="%.6g",
        delimiter="\t",
        header="radius\t" + header + "[km/s]",
    )
    numpy.savetxt(
        "mwmodel_sigmaz.txt",
        numpy.column_stack((radii, meanv2[:, :, 2] ** 0.5)),
        fmt="%.6g",
        delimiter="\t",
        header="radius\t" + header + "[km/s]",
    )
    numpy.savetxt(
        "mwmodel_sigmaphi.txt",
        numpy.column_stack((radii, (meanv2[:, :, 1] - meanv[:, :, 1] ** 2) ** 0.5)),
        fmt="%.6g",
        delimiter="\t",
        header="radius\t" + header + "[km/s]",
    )
    numpy.savetxt(
        "mwmodel_meanvphi.txt",
        numpy.column_stack((radii, meanv[:, :, 1])),
        fmt="%.6g",
        delimiter="\t",
        header="radius\t" + header + "[km/s]",
    )
...
def writeVelocityDistributions(model):
    ...
    for ip, point in enumerate(points):
        for iv in range(3):
            numpy.savetxt(
                "mwmodel_vdf_R%g_z%g_v%s.txt" % (point[0], point[2], ["R", "phi", "z"][iv]),
                numpy.column_stack(
                    [gridv] + [rho[ip][ic] * result[iv][ip][ic](gridv) * 1e-9 for ic in range(len(result[iv][ip]))]
                ),
                fmt="%.6g",
                delimiter="\t",
                header="v\t" + header + "[Msun/pc^3/(km/s)]",
            )
...
if __name__ == "__main__":
    ...
    dfStellar = agama.DistributionFunction(dfBulge, dfYoungDisk, dfMiddleDisk, dfOldDisk, dfHighADisk, dfStellarHalo)
    time1 = time.time()
    ...
    agama.writeSnapshot(
        "mwmodel_dm_final.nbody",
        agama.GalaxyModel(potential=model.potential, df=dfDarkHalo, af=model.af).sample(750000),
        format,
    )
    agama.writeSnapshot("model_stars_final.nbody", modelStars.sample(200000), format)
