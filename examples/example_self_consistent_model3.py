...
def writeRotationCurve(filename, potentials, names):
    ...
    numpy.savetxt(
        filename,
        numpy.column_stack((radii, vtot2**0.5, vcomp2**0.5)),
        fmt="%.6g",
...
if __name__ == "__main__":
    ...
    agama.writeSnapshot(
        "model_disk_final", agama.GalaxyModel(potential=model.potential, df=dfDisk, af=model.af).sample(1600000), format
    )
    print("Sampling bulge DF")
    agama.writeSnapshot(
        "model_bulge_final",
        agama.GalaxyModel(potential=model.potential, df=dfBulge, af=model.af).sample(400000),
        format,
    )
    print("Sampling halo DF")
    # note: use a 10x larger particle mass for halo than for bulge/disk
    agama.writeSnapshot(
        "model_halo_final", agama.GalaxyModel(potential=model.potential, df=dfHalo, af=model.af).sample(3000000), format
    )
    ...
    numpy.savetxt(
        ...
        header="R Sigma rho(R,z=0) sigma_R sigma_phi sigma_z v_phi ToomreQ",
        fmt="%.6g",
    )
