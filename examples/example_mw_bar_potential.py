...
def makeXBar(**params):
    ...
    def density(xyz):
        ...
        a = (
            ((abs(xyz[:, 0]) / x0) ** cperp + (abs(xyz[:, 1]) / y0) ** cperp) ** (cpar / cperp)
            + (abs(xyz[:, 2]) / z0) ** cpar
        ) ** (1 / cpar)
...
def makeLongBar(**params):
    ...
    def density(xyz):
        ...
        return (
            densityNorm
            / numpy.cosh(xyz[:, 2] / scaleHeight) ** 2
            * numpy.exp(
                -(a**cpar)
                - (R / outerCutoffRadius) ** outerCutoffStrength
...
def makeCMC(mass, scaleRadius, scaleHeight, axisRatioY):
    ...
    def density(xyz):
        return norm * numpy.exp(
            -((xyz[:, 0] ** 2 + (xyz[:, 1] / axisRatioY) ** 2) ** 0.5) / scaleRadius - abs(xyz[:, 2]) / scaleHeight
        )
...
def makePotentialModel():
    ...
    params_disk = [1.03063359e09, 4.75409497e00, 4.68804907e00, 1.51100601e-01, 1.53608780e00, 7.15915848e-01]
...
if __name__ == "__main__":
    ...
    print(
        "This script provides a realistic model for the bar, but not for the rest of the Galaxy; "
        "a better Milky Way potential with the same bar is constructed by example_mw_potential_hunter24.py"
    )
    print(
        "Created MW potential: total mass in stars=%.3g Msun, halo=%.3g Msun" % (pot[0].totalMass(), pot[1].totalMass())
    )
    ...
    orbits = agama.orbit(potential=pot, ic=ic, time=10.0, trajsize=1000, Omega=Omega)[:, 1]
    sina, cosa = numpy.sin(bar_angle), numpy.cos(bar_angle)
    rmax = 10.0  # plotting range
    cmap = plt.get_cmap("mist")
    for i, o in enumerate(orbits):
        ax[1].plot(
            o[:, 0] * cosa - o[:, 1] * sina, o[:, 0] * sina + o[:, 1] * cosa, color=cmap(i * 1.0 / numorbits), lw=0.5
        )
    ...
    Sigma = pot[0].projectedDensity(
        gridxy, gamma=-bar_angle
    )  # surface density for a stellar component rotated by bar_angle
