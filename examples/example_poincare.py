...
def init_axes(arg=None):
    ...
    vval = (
        numpy.maximum(0, 2 * E - 2 * pot.potential(numpy.column_stack((grid, grid * 0, grid * 0))) - (Lz / grid) ** 2)
        ** 0.5
    )
    zval = numpy.hstack(
        [
            ...
            numpy.array(
                [
                    scipy.optimize.brentq(lambda z: pot.potential(xx, 0, z) - E + 0.5 * (Lz / xx) ** 2, 0, xmax)
                    for xx in grid[1:-1]
                ]
            ),
            0,
...
def add_point(event):
    if event.inaxes is not axpss or event.button != 3:
        return
