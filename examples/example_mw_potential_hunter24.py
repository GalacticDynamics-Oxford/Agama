...
def makePotentialModel():
    ...
    params_disk = [
        dict(
            type="Disk", surfaceDensity=1.332e9, scaleRadius=2.0, scaleHeight=0.3, innerCutoffRadius=2.7, sersicIndex=1
        ),  # thin stellar disk
        dict(
            type="Disk", surfaceDensity=8.97e8, scaleRadius=2.8, scaleHeight=0.9, innerCutoffRadius=2.7, sersicIndex=1
        ),
    ]  # thick stellar disk

    params_gas = [
        dict(
            type="Disk", surfaceDensity=5.81e7, scaleRadius=7, scaleHeight=-0.085, innerCutoffRadius=4, sersicIndex=1
        ),  # HI gas disk
        dict(
            type="Disk", surfaceDensity=2.68e9, scaleRadius=1.5, scaleHeight=-0.045, innerCutoffRadius=12, sersicIndex=1
        ),
    ]  # molecular gas disk
...
""" % (filename_full, angle_bar, angle_bar + Omega_bar)
    with open(filename_rotating, "w") as f:
        f.write(
            """# Milky Way potential from Hunter+ 2024 with added rotation
[Potential]
"""
            + rotation_bar
        )
    with open(filename_rotspiral, "w") as f:
        f.write(
            """# Milky Way potential from Hunter+ 2024 with added rotation and spiral arms
[Potential main]
%s
[Potential spiral]
file=%s
...
pot_plot = agama.Potential(agama.Potential(filename_full)[1], agama.Potential(filename_spiral))
...
ic = numpy.random.normal(size=(numorbits, 6)) * numpy.array([2.0, 0.0, 0.4, 50.0, 40.0, 30.0])
ic[:, 0] += -6.0
ic[:, 4] += 220
...
times, orbits = agama.orbit(potential=pot_rotspiral, ic=ic, time=10.0, trajsize=1001).T
...
for i in range(numorbits):
    ...
    ax[1].plot(
        ...
        -orbits[i][:, 0] * sina + orbits[i][:, 1] * cosa,
...
Sigma = pot_plot.projectedDensity(
    gridxy, gamma=-angle_bar
)  # surface density for a stellar component rotated by angle_bar
...
ax[1].contour(
    gridr,
    gridr,
    logSigma.reshape(len(gridr), len(gridr)).T,
    levels=numpy.linspace(-8, 0, 17),
    colors="k",
    zorder=5,
    linewidths=[2, 1],
    linestyles="solid",
