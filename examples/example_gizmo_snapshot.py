...
def fitPotential(
    sim_name, nsnap=600, symmetry="a", subsample_factor=1, rmax_sel=600, rmax_ctr=10, rmax_exp=500, save_coords=True
):
    """
    ...
    dist = ut.particle.get_distances_wrt_center(
        part, species=["gas", "star", "dark"], rotation=part.host_rotation_tensors[0], total_distance=True
    )
    dist_vectors = ut.particle.get_distances_wrt_center(
        part, species=["gas", "star", "dark"], rotation=part.host_rotation_tensors[0]
    )
    ...
    new_rot = ut.particle.get_principal_axes(part, "star", part_indices=ctr_indices, print_results=False)
    ...
    dist_vectors = ut.particle.get_distances_wrt_center(
        part, species=["star", "gas", "dark"], center_position=new_ctr, rotation=new_rot["rotation.tensor"]
    )
    ...
    print(
        "{0:.3g} of {1:.3g} solar masses are hot gas to be modeled with multipole".format(m_gas_hot.sum(), m_gas.sum())
    )
    ...
    p_dark = agama.Potential(
        type="multipole", particles=(pos_pa_dark, m_dark), lmax=4, symmetry=symmetry, rmin=0.1, rmax=rmax_exp
    )
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Constructs and saves a hybrid "
    ...
    parser.add_argument(
        "--symmetry",
        help="s' for spherical, 'a' for axisymmetric, 't' for triaxial, 'n' for none (see table 2 of Agama docs)",
        default="a",
    )
    parser.add_argument(
        "--subfactor", type=int, help="factor by which to subsample snapshot particles (for speedup/testing)", default=1
    )
    parser.add_argument(
        "--rsel",
        type=float,
        help="max radius in kpc to select particles for the model (should be larger than rmaxc, rmaxm)",
        default=600,
    )
    parser.add_argument(
        "--rmaxc", type=float, help="max radius in kpc to select particles for centering the model", default=10
    )
    parser.add_argument(
        "--rmaxe", type=float, help="max radial extent in kpc of the hybrid potential expansion", default=400
    )
