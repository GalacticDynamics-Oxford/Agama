...
class SphericalModel:
    ...
    def __init__(self, Radii, show=False):
        ...
        self.ampl = numpy.exp(
            scipy.optimize.leastsq(
                lambda params: numpy.log(numpy.sum(numpy.exp(params) * basis, axis=1)) - rhs, numpy.ones(K)
            )[0]
        )
        if show:
            ...
            binsLogR = numpy.linspace(
                numpy.log(numpy.min(Radii)), numpy.log(numpy.max(Radii)), 2 * int(len(Radii) ** 0.5)
            )
            hist = numpy.histogram(numpy.log(Radii), bins=binsLogR)[0] / (
                len(Radii) * 2 * numpy.pi * (binsLogR[1:] - binsLogR[:-1])
            )
    ...
    def surfaceDensity(self, R):
        ...
        return numpy.sum(
            self.ampl * numpy.exp(-0.5 * R[:, None] ** 2 / self.sigma**2) / (2 * numpy.pi * self.sigma**2), axis=1
        )

    def rho(self, r):
        """Return 3d density rho(r)"""
        return numpy.sum(
            self.ampl * numpy.exp(-0.5 * r[:, None] ** 2 / self.sigma**2) / (2 * numpy.pi * self.sigma**2) ** 1.5,
            axis=1,
        )
    ...
    def sampleZ(self, R, numSamples):
        ...
        S = numSamples
        P = (
            self.ampl * numpy.exp(-0.5 * R[:, None] ** 2 / self.sigma**2) / (2 * numpy.pi * self.sigma**2)
        )  # shape: (N,K)
        ...
        rho = numpy.sum(
            self.ampl * numpy.exp(-0.5 * r2[:, :, None] / self.sigma**2) / (2 * numpy.pi * self.sigma**2) ** 1.5, axis=2
        )
...
def sampleMissingData(particles, numSamples):
    ...
    print(
        "Resample %d input particles into %d internal samples (vmax=%g, vstd=%g, vmad=%g, vmed=%g, adopted sigma=%g)"
