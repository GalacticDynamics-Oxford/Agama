...
def test(gpot, Gpot, Apot, test_hessian=False):
    ...
    print(
        "G acceleration vs force:  %.3g" % numpy.max(abs(Gpot.acceleration(points.T).T / Gpot.force(points) / fu - 1))
    )
    print(
        "A acceleration vs force:  %.3g" % numpy.max(abs(Apot.acceleration(points.T).T / Apot.force(points) / fu - 1))
    )
    ...
    print(
        "G density [gala] vs [agama]: %.3g"
        % numpy.max(abs(Gpot.density(points.T) / Gpot.agamadensity(points) / du - 1))
    )
    print(
        "A density [gala] vs [agama]: %.3g"
        % numpy.max(abs(Apot.density(points.T) / Apot.agamadensity(points) / du - 1))
    )
    ...
    t5 = time.time()
    print(
        "gala  orbit integration for g (native): %.4g s" % (t1 - t0)
        + ", G (wrapper): %.4g s" % (t2 - t1)
        + ", A: %.4g s" % (t3 - t2)
    )
    ...
    maxrad = numpy.max(
        numpy.sum(a_orb_A**2, axis=0) ** 0.5, axis=0
    )  # normalization factor for relative deviations in position
