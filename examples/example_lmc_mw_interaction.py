...
l, b, pml, pmb = agama.transformCelestialCoords(
    agama.fromICRStoGalactic, ra * numpy.pi / 180, dec * numpy.pi / 180, pmra, pmdec
)
...
paramHalo = dict(
    type="Spheroid", densityNorm=1.35e7, scaleRadius=14, outerCutoffRadius=300, cutoffStrength=4, gamma=1, beta=3
)
...
gmHalo = agama.GalaxyModel(
    potMWsph, agama.DistributionFunction(type="quasispherical", density=densMWhalo, potential=potMWsph)
)
...
potLMC = agama.Potential(
    type="spheroid", mass=massLMC, scaleradius=radiusLMC, outercutoffradius=radiusLMC * 10, gamma=1, beta=3
)
...
print(
    "Integrating the trajectories of %i stars in the Milky Way halo "
    "in the time-dependent potential of the LMC + MW" % len(ic)
)
if Nstars < 1e6:
    print(
        "To increase the resolution and reduce the Poisson noise in maps, run the script "
        "with a larger number of orbits, e.g. 1e6 (provide the number in the command line)"
    )

fc = numpy.vstack(agama.orbit(potential=potTotal, ic=ic, time=Tcurr - Trewind, timestart=Trewind, trajsize=1)[:, 1])
...
def showcolorbar(ax, label, vmin, vmax):
    ax.imshow(
        numpy.linspace(0, 1, 256).reshape(-1, 1),
