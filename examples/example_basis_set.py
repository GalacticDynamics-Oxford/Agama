...
def createPlummer(
    nbody=100000, rscale=1.2, p=0.8, q=0.6, alpha=1.0, beta=2.0, gamma=3.0, offset=numpy.array([0.1, 0.2, 0.3])
):
    ...
    radius = rscale * (numpy.random.random(size=nbody) ** (-2.0 / 3) - 1) ** -0.5
    ...
    xyz = numpy.column_stack(
        (radius * sintheta * numpy.cos(phi), radius * sintheta * numpy.sin(phi) * p, radius * costheta * q)
    )
    rotmat = agama.makeRotationMatrix(alpha, beta, gamma)
    xyzrot = xyz.dot(rotmat) + offset
    norm = rscale**2 * 3.0 / (4 * numpy.pi * p * q)
...
t0 = time.time()
pot_agama_nbody = agama.Potential(
    type="BasisSet", nmax=nmax, lmax=lmax, eta=1.0, r0=r0, symmetry="none", particles=(particles, masses)
)
pot_agama_nbody.export("example_basis_set_nbody.ini")
pot_agama_densfnc = agama.Potential(
    type="BasisSet", nmax=nmax, lmax=lmax, eta=1.0, r0=r0, symmetry="none", density=densfnc
)
...
print(
    "potential difference between BasisSet potentials initialized from nbody and analytic density: %g"
    % numpy.mean((Phi_agama_nbody / Phi_agama_densfnc - 1) ** 2) ** 0.5
)
print(
    "acceleration difference between BasisSet potentials initialized from nbody and analytic density: %g"
    % numpy.mean(
        numpy.sum((acc_agama_nbody - acc_agama_densfnc) ** 2, axis=1) ** 0.5
        / numpy.sum(acc_agama_densfnc**2, axis=1) ** 0.5
    )
)
print(
    "density difference between the true density and BasisSet initialized from analytic density: %g"
    % numpy.mean((den_agama_densfnc / den_true - 1) ** 2) ** 0.5
)
print(
    "density difference between the true density and BasisSet initialized from nbody snapshot: %g"
...
t0 = time.time()
pot_multipole_nbody = agama.Potential(type="Multipole", lmax=lmax, symmetry="none", particles=(particles, masses))
pot_multipole_densfnc = agama.Potential(
    type="Multipole", lmax=lmax, rmin=0.01, rmax=100, symmetry="none", density=densfnc
)
print("time for constructing and evaluating agama Multipole potentials: %.3g s" % (time.time() - t0))
print(
    "density difference between the true density and Multipole initialized from analytic density: %g"
    % numpy.mean((pot_multipole_densfnc.density(points) / den_true - 1) ** 2) ** 0.5
)
print(
    "density difference between the true density and Multipole initialized from nbody snapshot: %g"
...
def testGalpy():
    ...
    Acos[:, :, 0] *= 0.5
    Asin[:, :, 0] *= 0.5  # by some strange convention, m=0 terms are doubled in galpy
    ...
    pointscyl = [
        ...
        points[:, 2],
        numpy.arctan2(points[:, 1], points[:, 0]),
    ]
    ...
    print("time for constructing galpy scf potential: %.3g s, evaluating it: %.3g s" % (t1 - t0, time.time() - t2))
    print(
        "potential difference between agama(native) and agama(from galpy): %g"
        % numpy.mean((Phi_agama_galpy / Phi_agama_densfnc - 1) ** 2) ** 0.5
    )
    print(
        "potential difference between galpy(native) and agama(from galpy): %g"
        % numpy.mean((Phi_agama_galpy / Phi_galpy_native - 1) ** 2) ** 0.5
    )
    print(
        "acceleration difference between agama(native) and agama(from galpy): %g"
        % numpy.mean(
            numpy.sum((acc_agama_galpy - acc_agama_densfnc) ** 2, axis=1) ** 0.5
            / numpy.sum(acc_agama_nbody**2, axis=1) ** 0.5
        )
    )
    print(
        "acceleration difference between galpy(native) and agama(from galpy): %g"
        % numpy.mean(
            numpy.sum((acc_agama_galpy - acc_galpy_native) ** 2, axis=1) ** 0.5
...
def testGala():
    ...
    t0 = time.time()
    Acos, Asin = gala.potential.scf.compute_coeffs_discrete(particles, mass=masses, nmax=nmax, lmax=lmax, r_s=r0)
    ...
    print("time for constructing gala scf potential: %.3g s, evaluating it: %.3g s" % (t1 - t0, time.time() - t2))
    print(
        "potential difference between agama(native) and agama(from gala): %g"
        % numpy.mean((Phi_agama_gala / Phi_agama_nbody - 1) ** 2) ** 0.5
    )
    print(
        "potential difference between  gala(native) and agama(from gala): %g"
        % numpy.mean((Phi_agama_gala / Phi_gala_native - 1) ** 2) ** 0.5
    )
    print(
        "acceleration difference between agama(native) and agama(from gala): %g"
        % numpy.mean(
            numpy.sum((acc_agama_gala - acc_agama_nbody) ** 2, axis=1) ** 0.5
            / numpy.sum(acc_agama_nbody**2, axis=1) ** 0.5
        )
    )
    print(
        "acceleration difference between  gala(native) and agama(from gala): %g"
        % numpy.mean(
            numpy.sum((acc_agama_gala - acc_gala_native) ** 2, axis=1) ** 0.5
