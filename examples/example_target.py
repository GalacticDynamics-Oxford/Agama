...
mat_dens, mat_shell, mat_losvd = agama.orbit(
    potential=pot, ic=ic, time=100 * pot.Tcirc(ic), targets=[tar_dens, tar_shell, tar_losvd]
)
...
print(
    "Total: %6.1f %11.6f %11.6f %11.6f %11.6f"
...
)  # similarly, average between theta and z
...
print(
    "Total: %6.1f %11.6f %11.6f %11.6f %11.6f %11.6f"
...
for i in range(numaper):
    ax[i].plot(
        gridvplot, agama.bsplineInterp(degree, gridv, losvd_orb.reshape(numaper, -1)[i], gridvplot), label="orbits"
    )
    ax[i].plot(
        gridvplot, agama.bsplineInterp(degree, gridv, losvd_df.reshape(numaper, -1)[i], gridvplot), label="smooth DF"
    )
    ax[i].plot(
        gridvplot, agama.bsplineInterp(degree, gridv, losvd_nb.reshape(numaper, -1)[i], gridvplot), label="N-body"
    )
