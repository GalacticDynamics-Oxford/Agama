...
def createModel(iniFileName):
    ...
    if Omega != 0:
        try:
            ...
            print(
                "Omega=%.3g => corotation radius: %.3g"
                % (Omega, brentq(lambda r: model.potential.force(r, 0, 0)[0] / r + Omega**2, 1e-10, 1e10, rtol=1e-4))
            )
    ...
    for name, value in sec_comp.items():
        ...
        targetDensityParams = dict(
            [
                param
                for param in value.items()
                if param[0].lower()
                in ("type", "gridr", "gridz", "lmax", "mmax", "stripsperpane", "axisratioy", "axisratioz")
            ]
        )
        targets.append(agama.Target(**targetDensityParams))
        if "kinemgrid" in value:
            targetKinemParams = {"type": "KinemShell", "gridr": value["kinemgrid"], "degree": int(value["kinemdegree"])}
...
def runComponent(comp, pot):
    ...
    if hasattr(comp, "nbody"):  # record orbit trajectories
        result = agama.orbit(
            potential=pot, ic=comp.ic, time=comp.inttime, Omega=comp.Omega, targets=comp.targets, dtype=object
        )
        traj = result[-1]
    else:
        result = agama.orbit(potential=pot, ic=comp.ic, time=comp.inttime, Omega=comp.Omega, targets=comp.targets)
    ...
    for t in range(len(matrix)):
        ...
        for c, d in enumerate(delta):
            if abs(d) > norm[c]:
                print(
                    "Constraint %i:%i not satisfied: %s, val=%.4g, dif=%.4g" % (t, c, comp.targets[t][c], rhs[t][c], d)
                )
    print(
        "Entropy: %f, # of useful orbits: %i / %i"
        % (
            -sum(weights * numpy.log(weights + 1e-100)) / mass + numpy.log(avgweight),
            len(numpy.where(weights >= avgweight)[0]),
            len(comp.ic),
        )
    )
    ...
    if hasattr(comp, "nbody"):
        ...
        if not status:
            raise RuntimeError("Failed to produce output N-body model")
        comp.nbodymodel = particles

    # output
    comp.weights = weights
    comp.densitydata = result[0]
    if len(comp.targets) >= 2:
        comp.kinemdata = result[1]
...
if __name__ == "__main__":
    ...
    for name, comp in model.components.items():
        ...
        if hasattr(comp, "densitydata"):
            args["densitydata"] = comp.densitydata
        if hasattr(comp, "kinemdata"):
            args["kinemdata"] = comp.kinemdata
        try:
