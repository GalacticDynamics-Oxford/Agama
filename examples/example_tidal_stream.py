...
while time < tend:
    ...
    time_center, orbit_center = orbitDF(
        ic=r_center, time=tupd, timestart=time, trajsize=round(tupd / tau) + 1, mass=r_mass[-1]
    )
    ...
    pot_total = agama.Potential(
        pot_host, agama.Potential(potential=pot_sat, center=numpy.column_stack((time_center, orbit_center)))
    )
    ...
    r_bound = (
        pot_sat.potential(r_xv[:, 0:3] - r_center[0:3]) + 0.5 * numpy.sum((r_xv[:, 3:6] - r_center[3:6]) ** 2, axis=1)
        < 0
    )
    ...
    if pyfalcon:
        ...
        while f_time < tupd:
            ...
            f_center[0:3] += (
                tau * f_center[3:6]
            )  # linearly extrapolate from the previous timestep to get the first estimate
            Rmax = 10.0
            ...
            for i in range(10):
                ...
                if numpy.sum(f_bound) <= 1 or all(f_center == prev_f_center):
                    break
    ...
    if plot or time == tend:
        ...
        bx2.text(
            0.5, 0.1, "full N-body" + ("" if pyfalcon else " (n/a)"), ha="center", va="bottom", transform=bx2.transAxes
        )
        ax1.scatter(
            r_xv[:, 0],
            r_xv[:, 1],
            ...
            vmin=0,
            vmax=1,
        )  # blue: bound, red: unbound particles
