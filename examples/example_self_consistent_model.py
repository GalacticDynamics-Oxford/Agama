...
def writeRotationCurve(filename, potentials):
    ...
    numpy.savetxt(
        filename,
        numpy.column_stack((radii, vtot, vcomp)),
        fmt="%.6g",
        delimiter="\t",
...
def writeSurfaceDensityProfile(filename, model):
    ...
    radii = numpy.hstack(([1.0 / 8, 1.0 / 4], numpy.linspace(0.5, 16, 32), numpy.linspace(18, 30, 7)))
    xy = numpy.column_stack((radii, radii * 0))
    Sigma = (
        model.moments(xy, dens=True, vel=False, vel2=False, separate=True) * 1e-6
    )  # convert from Msun/Kpc^2 to Msun/pc^2
    numpy.savetxt(
        filename,
        numpy.column_stack((radii, Sigma)),
        fmt="%.6g",
        delimiter="\t",
...
def writeVerticalDensityProfile(filename, model):
    ...
    dens = (
        model.moments(xyz, dens=True, vel=False, vel2=False, separate=True) * 1e-9
    )  # convert from Msun/Kpc^3 to Msun/pc^3
    numpy.savetxt(
        filename,
        numpy.column_stack((height, dens)),
        fmt="%.6g",
        delimiter="\t",
...
def writeVelocityDispersionProfile(filename, model):
    ...
    numpy.savetxt(
        filename,
        numpy.column_stack((radii, numpy.dstack((vel2[:, :, 0:3] ** 0.5, vel[:, :, 1:2])).reshape(len(radii), -1))),
        fmt="%.6g",
        delimiter="\t",
        header="Radius[Kpc]\t"
...
def writeVelocityDistributions(filename, model):
    ...
    numpy.savetxt(
        filename,
        numpy.column_stack((gridv, splvx(gridv), splvy(gridv), splvz(gridv))),
        fmt="%.6g",
        delimiter="\t",
        header="V\tf(V_x)\tf(V_y)\tf(V_z) [1/(km/s)]",
    )
...
def printoutInfo(model, iteration):
    ...
    pt1 = (solarRadius, 0, 1)
    print(
        "Disk total mass=%g Msun, rho(Rsolar,z=0)=%g, rho(Rsolar,z=1kpc)=%g Msun/pc^3"
        % (densDisk.totalMass(), densDisk.density(pt0) * 1e-9, densDisk.density(pt1) * 1e-9)
    )  # per pc^3, not kpc^3
    print(
        "Halo total mass=%g Msun, rho(Rsolar,z=0)=%g, rho(Rsolar,z=1kpc)=%g Msun/pc^3"
        % (densHalo.totalMass(), densHalo.density(pt0) * 1e-9, densHalo.density(pt1) * 1e-9)
    )
    print(
        "Potential at origin=-(%g km/s)^2, total mass=%g Msun"
    ...
    writeRotationCurve(
        "rotcurve_" + iteration,
        (
            model.potential[1],  # disk potential (CylSpline)
...
if __name__ == "__main__":
    ...
    print(
        "\033[1;33m**** STARTING MODELLING ****\033[0m\nInitial masses of density components: "
        "Mdisk=%g Msun, Mbulge=%g Msun, Mhalo=%g Msun, Mgas=%g Msun"
        % (
            densityStellarDisk.totalMass(),
            densityBulge.totalMass(),
            densityDarkHalo.totalMass(),
            densityGasDisk.totalMass(),
        )
    )
    ...
    print(
        ...
        % (
            dfStellar.totalMass(),
            dfThinDisk.totalMass(),
            dfThickDisk.totalMass(),
            dfStellarHalo.totalMass(),
            dfBulge.totalMass(),
            dfHalo.totalMass(),
        )
    )
    ...
    agama.writeSnapshot(
        "model_dm_final", agama.GalaxyModel(potential=model.potential, df=dfHalo, af=model.af).sample(800000), format
    )
    print("Writing a complete DF-based N-body model for the stellar bulge, disk and halo")
    agama.writeSnapshot(
        "model_stars_final",
        agama.GalaxyModel(potential=model.potential, df=dfStellarAll, af=model.af).sample(200000),
        format,
    )
    ...
    agama.writeSnapshot(
        "model_gas_final", model.components[3].density.sample(24000, potential=model.potential, beta=0, kappa=1), format
    )
