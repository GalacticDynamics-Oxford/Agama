...
def derlogexpm1overx(x):
    ...
    return numpy.where(abs(x) < 1e-2, 0.5 + x * (1.0 / 12 - (1.0 / 720) * x**2), -1 / x - 1 / numpy.expm1(-x))
...
def getLyapunovExponent(times, logDeviationVector, orbitalPeriod, ax, color):
    ...
    for i in range(size):
        ti = times[i]
        vi = logDeviationVector[i]
        tp = times[i + 1]
        vp = logDeviationVector[i + 1]
...
def onclick(event):
    ...
    (time, traj), devvec, (lyap, tch) = agama.orbit(
        potential=pot, ic=ic[which], time=numPeriods * Torb[which], trajsize=0, der=True, lyapunov=True, dtype=float
    )
    ...
    print(
        "Normalized Lyapunov exponent (C++/Python implementations): %.10g/%.10g; "
    ...
    ax1.plot(
        time / Torb[which],
        curve(time, Lyap, intercept, Tch) - numpy.log(time),
        c="b",
        lw=1.5,
        dashes=[2, 1],
        label="fitted curve",
