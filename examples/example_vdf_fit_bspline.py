...
if addErrors:
    ...
    print(
        "Assuming no measurement errors; run the script with a non-empty command-line argument "
...
def getampl(params):
    ...
    amid = (
        1
        - integ[2 : 2 + nparams // 2].dot(params[0 : nparams // 2])
        - integ[3 + nparams // 2 : 3 + nparams].dot(params[nparams // 2 :])
    ) / integ[nparams // 2 + 3]
...
axes[-1].set_ylim(
    maxloglike - max(nparams * 2, 20), maxloglike
...
plt.plot(
    gridll,
    1 / scipy.special.gamma(0.5 * nparams) * numpy.maximum(0, -gridll) ** (0.5 * nparams - 1) * numpy.exp(gridll),
    "r",
    lw=2,
)
...
plt.plot(
    gridv_plot[::2].repeat(2)[1:-1], hist.repeat(2), c="r", label="observed%s" % (" (with errors)" if addErrors else "")
)
...
plt.fill_between(
    gridv_plot, numpy.percentile(profs, 16, axis=0), numpy.percentile(profs, 84, axis=0), alpha=0.5, color="g", lw=0
)
...
plt.text(
    0.98,
    0.98,
    r"$\overline{v}=%.3f\pm%.3f$" % (numpy.mean(meanv), numpy.std(meanv)),
    ha="right",
    va="top",
...
plt.text(
    0.98,
    0.92,
    r"$\sigma=%.3f\pm%.3f$" % (numpy.mean(sigmav), numpy.std(sigmav)),
    ha="right",
    va="top",
    transform=plt.gca().transAxes,
)
