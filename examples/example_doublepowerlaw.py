#!/usr/bin/python
__docstring__ = (
    "Find parameters of a DoublePowerLaw DF f(J) that best correspond "
...
gridRadius = 10 ** numpy.array(
    [scipy.optimize.brentq(lambda logr: density.enclosedMass(10**logr) - mass, -50, 50) for mass in gridMass]
)
...
vt0sq = (xv0[:, 0] * xv0[:, 4] - xv0[:, 1] * xv0[:, 3]) ** 2 / (
    xv0[:, 0] ** 2 + xv0[:, 1] ** 2
)  # v_phi^2 for each point
...
bounds = (
    ...
    numpy.array([+100, 3.0, 20.0, 5.0, 3.0, 3.0, 3.0, 3.0, +100, 20.0]),
)
...
print(
    numpy.column_stack(
        [gridRadius, rhs[0:gridSize], valden0, rhs[gridSize : 2 * gridSize], valden2, rhsBeta, 1 - valvtsq / valvrsq]
    )
)
print(
    "penalty for l=0: %g, l=2: %g, beta: %g"
    % (
        sum(deviation[0:gridSize] ** 2),
        sum(deviation[gridSize : 2 * gridSize] ** 2),
        sum(deviation[2 * gridSize :] ** 2),
    )
)
...
gridParams = dict(
    rminSph=gridRadius[1] * 0.5, rmaxSph=gridRadius[-1], sizeRadialSph=gridSize, lmaxAngularSph=(0 if spherical else 8)
)
...
ax[0].plot(
    gridRadius,
    gridRadius**3 * density.density(gridMajor),
    color="m",
    label=r"$r^3 \rho_{\rm orig,major}$",
    dashes=[3, 2],
)
ax[0].plot(
    gridRadius,
    gridRadius**3 * density.density(gridMinor),
    color="y",
    label=r"$r^3 \rho_{\rm orig,minor}$",
    dashes=[3, 2],
)
...
ax[2].plot(
    gridRadius, velsqMinor[:, 1] ** 0.5, color="g", label=r"$\sigma_{\phi, \rm final}$"
)  # equal in x,y directions
