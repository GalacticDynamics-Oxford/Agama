...
def contraction(pot_dm, pot_bar, method="C20", beta_dm=0.0, rmin=1e-2, rmax=1e4):
    ...
    if method == "adiabatic":
        ...
        pot_total_sph = agama.Potential(
            type="multipole", potential=agama.Potential(pot_dm, pot_bar), lmax=0, rmin=0.1 * rmin, rmax=10 * rmax
        )
        ...
        sph_dens_bar = agama.Density(
            cumulmass=numpy.column_stack((gridr[valid_r], cumul_mass_bar[valid_r]))
        )  # sphericalized baryon density profile
        f_bar = 0.157  # cosmic baryon fraction; the formula is calibrated against simulations only for this value
        eta_bar = (
            cumul_mass_bar / cumul_mass_dm * (1.0 - f_bar) / f_bar
        )  # the last two terms account for transforming the DM mass into the corresponding baryonic mass in dark-matter-only simulations
        first_factor = 0.45 + 0.41 * (eta_bar + 0.98) ** 0.53
        dens_dm_orig = pot_dm.density(xyz)
        temp = sph_dens_bar.density(xyz) - eta_bar * dens_dm_orig * f_bar / (1.0 - f_bar)
        const_term = 0.41 * 0.53 * (eta_bar + 0.98) ** (0.53 - 1.0) * (1.0 - f_bar) / f_bar * temp
...
if __name__ == "__main__":
    ...
    agama.setUnits(mass=1.0, length=1.0, velocity=1.0)  # Msun, kpc, km/s
    # DM halo
    ...
    # stellar disks
    params_thin_disk = dict(type="Disk", surfaceDensity=7.31e8, scaleRadius=2.63, scaleHeight=0.3)
    params_thick_disk = dict(type="Disk", surfaceDensity=1.01e8, scaleRadius=3.8, scaleHeight=0.9)
    # gas disks
    params_HI_disk = dict(type="disk", surfaceDensity=5.3e7, scaleRadius=7.0, scaleHeight=0.085, innerCutoffRadius=4.0)
    params_H2_disk = dict(type="disk", surfaceDensity=2.2e9, scaleRadius=1.5, scaleHeight=0.045, innerCutoffRadius=12.0)
    # CGM
    params_CGM = dict(
        type="Spheroid",
        densityNorm=7.615e2,
    ...
    params_halo = dict(type="Spheroid", densityNorm=3.487e6, gamma=1, beta=3, scaleRadius=25.2)

    pot_baryon = agama.Potential(
        params_bulge, params_thin_disk, params_thick_disk, params_HI_disk, params_H2_disk, params_CGM
    )
    ...
    names = [
        "baryons",
        "initial halo",
        "contracted Cautun+2020",
        "contracted isotropic",
        "contracted radial",
        "contracted tangential",
    ]
    colors = ["k", "c", "g", "m", "r", "b"]
