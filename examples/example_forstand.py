...
Omega = float(
    args.get("OMEGA", 0)
)  # [REQ] pattern speed (relevant only for non-axisymmetric models) [km/s/length_unit]
...
multstep = float(
    args.get("MULTSTEP", 1.02)
)  # [OPT] multiplicative step for increasing/decreasing Upsilon during grid search
...
regul = float(
    args.get("REGUL", 1.0)
)  # [OPT] regularization parameter (larger => more uniform orbit weight distribution in models)
...
alpha_deg = float(
    args.get("ALPHA", 0.0)
)  # [REQ] azimuthal angle of viewing direction in the model coordinates (relevant only for non-axisym)
...
addnoise = (
    args.get(
        "ADDNOISE", "True"
    ).upper()  # [OPT] whether to add a realistic amount of noise in generating mock datacubes
    in ("TRUE", "T", "YES", "Y")
)
...
command = args.get(
    "DO", ""
).upper()  # [REQ] operation mode: 'RUN' - run a model, 'PLOT' - show the model grid and maps, 'TEST' - show diagnostic plots, 'MOCK' - create mock maps
variant = args.get(
    "VARIANT", "GH"
).upper()  # [OPT] choice between three ways of representing and fitting LOSVDs (see below)
if "HIST" not in variant and "GH" not in variant and "VS" not in variant:
    raise RuntimeError(
        'parameter "variant" should be one of "GH" (Gauss-Hermite moments), "VS" (classical moments - v & sigma), or "HIST" (LOSVD histograms)'
    )
fileResult = "results%s.txt" % variant  # [OPT] filename for collecting summary results for the entire model grid
seed = int(
    args.get("SEED", 99)
)  # [OPT] random seed (different values will create different realizations of mock data when do=MOCK, or initial conditions for the orbit library when do=RUN)
agama.setRandomSeed(seed)  # note that Agama has its own
numpy.random.seed(
    seed
)  # make things repeatable when generating mock data (*not* related to the seed for the orbit library)
...
filenameMGE = (
    "mge_i%.0f.txt" % incl
)  # [REQ] file with parameters of the MGE model for the surface density profile (if MGE is used)

### common parameters for kinematic datasets (though in principle they may also differ between them)
gridv = numpy.linspace(
    -500, 500, 26
)  # [REQ] the grid in model velocity space (will be multiplied by sqrt(Upsilon) when comparing to data)
...
hist_gridv = numpy.linspace(
    -400, 400, 17
)  # [OPT] velocity grid for the observed LOSVDs (boundaries of velocity bins, not centers!)

### parameters of the 1st kinematic dataset
gamma1 = (
    25.0 * numpy.pi / 180
)  # [REQ] CW rotation angle of the image-plane X axis relative to the line of nodes (=major axis for axisym.systems)
psf1 = (
    1.0  # [REQ] width of the Gaussian PSF ( may use more than one component: [ [width1, weight1], [width2, weight2] ] )
)
...
filenameGH1 = (
    "kinem_gh_i%.0f_lr.txt" % incl
)  # [REQ] Gauss-Hermite parametrization of observed LOSVDs (usually only one of these two files is given)
...
def makeMockKin(kinemParams, gridxy, nbins, filenameVorBin, filenameHist, filenameGH, filenameVS):
    ...
    gridx, gridy = agama.schwarzlib.makeGridForTargetLOSVD(
        apertures, kinemParams["psf"]
    )  # construct a suitable image-plane grid
    ...
    conv = numpy.linalg.solve(  # conversion matrix from the model B-splines into observed histograms
        agama.bsplineMatrix(hist_degree, hist_gridv), agama.bsplineMatrix(hist_degree, hist_gridv, degree, gridv)
    ).T
    ...
    ghm_err = numpy.std(
        agama.ghMoments(
            degree=degree, gridv=gridv, ghorder=ghorder, matrix=datacubes.reshape(-1, datacubes.shape[2])
...
if command == "MOCK":
    ...
    print(
        "Scaling N-body model to physical units: 1 length unit = %g arcsec = %g kpc, 1 velocity unit = %g km/s, 1 mass unit = %g Msun"
    ...
    makeMockKin(
        kinemParams1, numpy.linspace(-30.0, 30.0, 61), 150, filenameVorBin1, filenameHist1, filenameGH1, filenameVS1
    )
    # High-resolution dataset similar to AO-assisted IFU such as NIFS (2x2", pixel size 0.1")
    makeMockKin(
        kinemParams2, numpy.linspace(-1.0, 1.0, 21), 50, filenameVorBin2, filenameHist2, filenameGH2, filenameVS2
    )
...
try:
    ...
    print(
        "%s not found; you need to generate the mock data first, as explained at the beginning of this file"
        % filenameMGE
    )
    exit()
...
if densityParams["type"] == "DensityClassicTopHat" or densityParams["type"] == "DensityClassicLinear":
    ...
    print(
        "%s grid in elliptical radius: %s, axis ratios: y/x=%.3g, z/x=%.3g"
    ...
    print(
        "%s grid in spherical radius: %s, lmax=%i, mmax=%i"
...
if "HIST" in variant:
    ...
    datasets.append(
        agama.schwarzlib.KinemDatasetHist(
            density=densityStars,
    ...
    datasets.append(
        agama.schwarzlib.KinemDatasetGH(
            density=densityStars,
            tolerance=0.01,  # [REQ] relative error in fitting aperture mass constraints
    ...
    datasets.append(
        agama.schwarzlib.KinemDatasetVS(
            density=densityStars,
            tolerance=0.01,  # [REQ] relative error in fitting aperture mass constraints
...
pot_gal = agama.Potential(
    ...
    mmax=0 if symmetry[0] != "t" else 6,
    gridSizeR=40,
)  # mmax>0 only for triaxial systems
...
if command == "RUN":
    ...
    if "gridR" in densityParams:
        gridnodesmajor = densityParams["gridR"]
    if densityParams["type"] == "DensityCylindricalTopHat" or densityParams["type"] == "DensityCylindricalLinear":
        ...
        if "axisRatioZ" in densityParams:  # elliptical grid in DensityClassic[TopHat/Linear]
            multX = (densityParams["axisRatioY"] * densityParams["axisRatioZ"]) ** (
                -1.0 / 3
            )  # scaling along the X (major) axis
            multZ = densityParams["axisRatioZ"] * multX  # scaling along the Z (minor) axis
        else:
            multX = multZ = 1
        ax[2].plot(
            densityParams["gridr"] * multX,
            numpy.interp(densityParams["gridr"] * multX, gridrmajor, rhomajor),
            "b|",
            ms=6,
        )
        ax[2].plot(
            densityParams["gridr"] * multZ,
            numpy.interp(densityParams["gridr"] * multZ, gridrminor, rhominor),
            "r|",
            ms=6,
        )
    ...
    for name, pot in [
        ...
        vcirc = (
            -gridrmajor * pot.force(numpy.column_stack((gridrmajor, gridrmajor * 0, gridrmajor * 0)))[:, 0] * Upsilon
        ) ** 0.5
    ...
    chi2 = numpy.sum(
        tab[:, 10:-1], axis=1
    )  # chi2 values are stored separately for each dataset, but here we combine all of them except regularization penalty
