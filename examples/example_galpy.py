...
agama.setUnits(mass=1.0, length=8.0, velocity=220.0)  # Msun, kpc, km/s
...
def compare(ic, inttime, numsteps):
    ...
    print(
        "Time to integrate 1/10th of the orbit in galpy, using galpy-hybrid potential [2]: %.4g s" % (time.time() - dt)
    )
    ...
    print(
        "Time to integrate 1/10th of the orbit in galpy, using agama-hybrid potential [3]: %.4g s" % (time.time() - dt)
    )
    ...
    print(
        ...
        + "[6]-[7]=%g" % numpy.max(numpy.abs(a_orb_a_native - a_orb_a_hybrid)[: len(times1)])
    )  # should be zero
...
compare([0.5, 0.82, 0.28, 0, 1.0, 0], 100.0, 1000)
