...
target1 = agama.Target(
    ...
    gridr=agama.nonuniformGrid(25, 0.1, 20.0),
    axisratioy=0.8,
    axisratioz=0.6,
    stripsPerPane=2,
)
...
data1, data2, trajs = agama.orbit(potential=pot, ic=initcond, time=inttimes, dtype=object, targets=[target1, target2])
...
weights = agama.solveOpt(
    matrix=(data0.T, data1.T, datak.T),
    rhs=([mass], rhs1, rhs2),
...
for i, d in enumerate(delta1):
    if abs(d) > 1e-8:
        print("DensityConstraint %i not satisfied: %s, val=%.4g, rel.err=%.4g" % (i, target1[i], rhs1[i], d / rhs1[i]))
delta2 = datak.T.dot(weights)
for i, d in enumerate(delta2):
    if abs(d) > 1e-8:
        print("KinemConstraint %i not satisfied: %.4g" % (i, d))
...
if not status:
    ...
    trajs[indices] = agama.orbit(potential=pot, ic=initcond[indices], time=inttimes[indices], trajsize=trajsizes)
    status, result = agama.sampleOrbitLibrary(nbody, trajs, weights)
    if not status:
        print("Failed to produce output N-body model")
...
numpy.savez_compressed(
    "schwarzschild_model_data", ic=initcond, inttime=inttimes, weight=weights, data1=data1, data2=data2, cons1=rhs1
)

# store the orbit initial conditions and weights in a text file
numpy.savetxt(
    "schwarzschild_model_orbits",
