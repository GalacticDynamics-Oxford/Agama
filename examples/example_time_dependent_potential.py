...
p1 = agama.Potential(
    ...
    dict(type="plummer", mass=0.001, scaleradius=0, center=[-0.999, 0, 0]),
)  # Earth (a very massive one)
...
center1 = numpy.column_stack(
    (tt, cosomt * 0.001, sinomt * 0.001, 0 * tt, -om * sinomt * 0.001, om * cosomt * 0.001, 0 * tt)
)
center2 = numpy.column_stack(
    (tt, cosomt * -0.999, sinomt * -0.999, 0 * tt, -om * sinomt * -0.999, om * cosomt * -0.999, 0 * tt)
)
...
p4 = agama.Potential(type="KeplerBinary", mass=1, binary_sma=1.0, binary_q=1.0 / 999, binary_ecc=0)
...
ax[0].plot(-0.999, 0, "o", ms=10, c="k", markerfacecolor="none")
ax[0].plot(-0.999, 0, "+", ms=9, c="k")
