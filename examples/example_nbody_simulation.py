...
def runRestrictedNbody():
    ...
    time_center, orbit_center = agama.orbit(
        potential=g_pot, ic=c_center, time=sim_time, trajsize=num_intervals * num_subint + 1
    )
    cpot = c_pot
    snap = xv.copy()
    ...
    for i in range(num_intervals + 1):
        ...
        c_bound = (
            c_pot.potential(snap[:, 0:3] - orbit_center[i * num_subint, 0:3])
            + 0.5 * numpy.sum((snap[:, 3:6] - orbit_center[i * num_subint, 3:6]) ** 2, axis=1)
            < 0
        )
        time = i * interval
        ax.cla()
        ...
        ax.text(
            0.01,
            0.99,
            "time=%.4f, bound fraction=%.3f" % (time, numpy.sum(c_bound) * 1.0 / N),
            ha="left",
            va="top",
        ...
        xyz = numpy.column_stack(
            [numpy.repeat(grid, len(grid)), numpy.tile(grid, len(grid)), numpy.zeros(len(grid) ** 2)]
        )
        ...
        t_pot = agama.Potential(
            g_pot, agama.Potential(potential=cpot, center=numpy.column_stack((time_center, orbit_center)))
        )
        # compute the trajectories of all particles moving in the combined potential of the host galaxy and the moving satellite
        snap = numpy.vstack(
            agama.orbit(
                ic=snap, potential=t_pot, time=interval, timestart=time, trajsize=1, accuracy=1e-5, verbose=False
            )[:, 1]
        )
        # update the potential of the satellite (using a spherical monopole approximation)
        cpot = agama.Potential(
            type="multipole", particles=(snap[:, 0:3] - orbit_center[(i + 1) * num_subint, 0:3], m), symmetry="s"
        )
...
def runGyrfalcon():
    ...
    if os.path.isfile(endfile):
        os.remove(endfile)
    runstr = (
        "gyrfalcON %s %s eps=0.001 kmax=14 Nlev=3 fea=0.5 step=0.001953125 tstop=0.25 logstep=32 "
        "Grav=%g accname=agama accpars=0,%g accfile=%s"
    ) % (infile, outfile, agama.G, agama.G, "../" + potfile)
...
def runArepoOrGadget(code):
    ...
    with open(path + "/Template-Config.sh", "r") as f:
        ...
        if "AGAMA" in lines:
            ...
            subprocess.check_call(
                "patch -r -u -N -d %s -p 1 < example_nbody_simulation_%s.patch" % (path, code), shell=True
            )
            ...
            with open(agama_root + "/Makefile.local", "r") as mf:
                for line in mf.readlines():
                    if (
                        line.startswith("LINK_FLAGS_ALL")
                        or line.startswith("LINK_FLAGS_LIB_AND_EXE_STATIC")
                        or
                        # for arepo, which is a C (not C++) program, need to add another set of flags...
                        code == "arepo"
                        and line.startswith("LINK_FLAGS_EXE_STATIC_NONCPP")
                    ):
            ...
            with open(path + "/Makefile", "r") as mf:
                ...
                for i in range(len(lines)):
                    if lines[i].startswith("ifeq (EXTERNALGRAVITY_AGAMA"):
                        lines = (
                            lines[: i + 1]
                            + ["LIBS += %s/agama.a " % agama_root + " ".join(allflags) + "\n"]
                            + lines[i + 1 :]
                        )
                        break
    ...
    if not os.path.isfile(executable):
        ...
        with open("Config.sh", "w") as f:
            f.write("\n".join(macros) + "\n")
        print(
            "*** I am going to compile the executable %s/%s, " % (path, executable)
...
if choice == "3":
    runGyrfalcon()
if choice == "4":
    runArepoOrGadget("gadget4")
if choice == "5":
    runArepoOrGadget("arepo")
