...
def getaxes(pos, mass, radius=numpy.inf):
    ...
    while True:
        ...
        axesnew = (
            val[order] / numpy.prod(val) ** (1.0 / 3)
        ) ** 0.5  # updated axis ratios, normalized so that ax*ay*az=1
        if sum(abs(axesnew - axes)) < 0.01:
            break
        axes = axesnew
...
if __name__ == "__main__":
    ...
    if len(sys.argv) <= 1:
        exit("Provide file name or 'test' to use an internally created fiducial triaxial snapshot")
    if sys.argv[1] == "test":
        ...
        pos = numpy.column_stack(
            (radius * sintheta * numpy.cos(phi), radius * sintheta * numpy.sin(phi), radius * costheta)
        )
    ...
    for i in range(nbins):
        ...
        print("%.3g\t%.3g\t%.3f\t%.3f" % (binrad[i], numpy.sum(mass[filter]), axes[1] / axes[0], axes[2] / axes[0]))
