...
if __name__ in ("__main__", "__plot__"):
    ...
    if useRaga:
        ...
        cluster = ph4(
            converter, channel_type="sockets", mode="gpu", parameters=[("epsilon_squared", (0.01 | units.parsec) ** 2)]
        )
    ...
    for i, t in enumerate(times):
        ...
        print(
            "Evolved the system to time %.1f Myr" % t.value_in(units.Myr)
            ...
            + ", Etot=%.4g Msun*(km/s)^2"
            % (cluster.potential_energy + cluster.kinetic_energy).value_in(units.MSun * units.kms**2)
        )
        x = particles.x.value_in(units.parsec)
        y = particles.y.value_in(units.parsec)
        plt.figure(figsize=(6, 6))
        plt.scatter(
            x, y, marker=".", s=5 * particles.mass.value_in(units.MSun) ** 0.5, edgecolors=None, c="k", alpha=0.2
        )
        plt.xlim(-5, 5)
        plt.ylim(-5, 5)
