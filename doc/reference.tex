...
\item \ppp{cutoffStrength} [2] -- parameter controlling the steepness of the exponential cutoff in \ttt{Spheroid} or \ttt{Nuker} models.
...
\overline{v_i v_j} &\equiv \rho^{-1} \iiint \d ^3v \,v_i v_j\, f\big(\bJ(\bx,\bv)\big)\,S(\bx,\bv).
\end{align*}
...
This is not a cheap operation, as the integration requires $\gtrsim 10^3$ evaluation of DF and hence calls to the action finder; the computation of density is the major cost in self-consistent modelling (Section~\ref{sec:SCM}). The first moment of velocity may have only one non-zero component ($\overline{v_\phi}$); the tensor of second velocity moments is computed in cylindrical coordinates ($R,z,\phi$), and in axisymmetric systems may have at most 4 non-zero components $\big(\overline{\strut v_R^2}, \overline{\strut v_z^2}, \overline{\strut v_\phi^2}, \overline{\strut v_R v_z}\big)$, the latter is related to the tilt of velocity ellipsoid in the meridional plane).
...
\begin{align*}
f_\mathrm{proj}(x,y;v_z) \equiv \int_{z=-\infty}^\infty \d z\, \iint \d v_x\, \d v_y\, f\big(\bJ(\bx,\bv)\big)\,S(\bx,\bv) .
\end{align*}
...
\item Draw samples directly from the six-dimensional $\{\bx,\bv\}$ space, evaluating $f\big(\bJ(\bx,\bv)\big)\,S(\bx,\bv)$ with the help of an action finder. This is performed by the routine \ttt{samplePosVel}.
\end{itemize}
...
\texttt{surfaceDensity(points [, alpha, beta, gamma])} computes the integral of density along the line of sight at a 2d point (or an array of points) in the image plane, using an arbitrarily rotated coordinate system, whose orientation is specified by Euler angles $\alpha, \beta, \gamma$ (Section~\ref{sec:CoordinateDetails}).\\[2mm]
...
You may create an instance of \ttt{Density} class wrapping a user-defined \Python callback function:\\
\texttt{densfnc = agama.Density(fnc1)}\\
However, in this case \textit{all} routines in the \Cpp library work in a single-thread mode during the lifetime of this instance (if it was created as a local variable in a function, it will be destroyed upon return from the function, or it could be explicitly destroyed by the \texttt{del densfnc} statement). Thus it is advisable to construct temporary instances which only exist during the execution of a single statement, e.g.,\\
\texttt{print(agama.Density(fnc1).totalMass())}\\
Alternatively, one may directly pass the \Python callback function whenever a \ttt{Density} object is expected, such as the \ttt{Potential} and \ttt{DistributionFunction} constructors, without creating a wrapper instance of \texttt{Density} class: \\
\texttt{pot1 = agama.Potential(type="Multipole", density=fnc1)}
...
\texttt{projectedForce(points [, alpha, beta, gamma])} computes the integral of the force along the line of sight, similar to \texttt{surfaceDensity}. The input parameters are the 2d point (or an array of points) $X,Y$ in the image plane, whose orientation with respect to the intrinsic model coordinates is specified by Euler angles $\alpha, \beta, \gamma$, and the output contains the $X,Y$ components of force integrated along the $Z$ axis.\\[2mm]
...
\\[2mm]
Similarly to the \ttt{Density} class, one may provide a custom \Python function \texttt{myfnc} which returns the DF value at the given triplet of actions $\{J_r,J_z,J_\phi\}$ (again the calling convention is to process a 2d array of such triplets, even if with one row). Note that such a user-defined \Python function disables OpenMP parallelization throughout the library during its existence (in this example, during the existence of the composite DF \texttt{dfcomp}).
\\[2mm]
...
\texttt{sf = agama.SelectionFunction(point=(x0,y0,z0), radius=r0, eta=eta))}\\
is mathematically equivalent to\\
\texttt{sf = lambda x: numpy.exp( \\
\mbox{}~~~~-((x[:,0]-x0)**2 + (x[:,1]-y0)**2 + (x[:,2]-z0)**2) / r0**2)**(eta/2))}\\
...
\texttt{gm = agama.GalaxyModel(pot, df [, sf])}
