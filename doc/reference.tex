...
Moreover there is a wrapper class that turns any user-provided function $\Phi(r)$ with two known derivatives into a form compatible with the potential interface.
...
\!\!\!\!\exp\bigg[-\left(\frac{g(\bJ)}{J_\mathrm{cut}}\right)^\zeta\bigg]
...
\texttt{p.force(points)} computes the force, i.e., \textit{minus} the derivative of potential, returning a $N\times3$ array;\\
...
where \ppp{type} may be one of the following: \ppp{DoublePowerLaw}, \ppp{QuasiIsothermal}, \ppp{Exponential}, \ppp{QuasiIsotropic}, and the other parameters are specific to each type of DF (Section~\ref{sec:DF}), \\
...
The \ppp{QuasiIsotropic} DF (Section~\ref{sec:DFspherical}) is constructed from the provided instances of \ttt{Density} and \ttt{Potential}, using the Eddington inversion formula to create $f(E)$ and then a spherical action finder to convert it to an action-based form.
...
This class provides methods for computing DF moments and drawing position/velocity samples from it. These operations are rather expensive, and if the input consists of several points, the computation is internally parallelized using \texttt{OpenMP} (except if the DF is a user-defined Python function).\\[2mm]
...
constructs 1d velocity distribution functions at the given point(s) (Section~\ref{sec:VDF}), represented by cubic spline interpolators; for instance, one may plot them by as follows:\\
...
Any non-negative function $f(h)$ may serve as a spherical isotropic DF. One possible representation is provided by the \ttt{math::LogLogSpline} class -- an interpolating spline in doubly-logarithmically scaled coordinates (i.e., $\ln f (\ln h)$ is a cubic spline and is extrapolated linearly to small and large $h$). Such DFs are constructed, e.g., by routines \ttt{makeEddingtonDF} and \ttt{fitSphericalDF} defined in \texttt{galaxymodel_spherical.h}.
