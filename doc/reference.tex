...
Another useful method is \texttt{totalMass()}, with an obvious role; the mass may well be infinite, e.g., for a \ppp{type="NFW"} model. If called with a single argument, \texttt{totalMass(radius)} estimates the mass enclosed within the given \texttt{radius}.\\[1mm]
