...
The \Nemo plugin allows to use any \Agama potential as an external potential in \textsc{gyrfalcON} and other \Nemo programs (in a similar context as the \Amuse plugin). Actually, it is even more general, with the possibility of representing a time-dependent potential, using a rotating reference frame, shifting the origin of the potential, and adding a spatially uniform acceleration (the latter two features can also be time-dependent). 
In the simplest case, a \ttt{Multipole} or a \ttt{CylSpline} potential is specified as a single file with expansion coefficients, using the command-line argument \texttt{accfile=\dots}. Alternatively, if the potential contains several components or is not an expansion, one needs to create an INI file with potential parameters defined in sections \ppp{[Potential1]}, \ppp{[Potential whatever]}, \dots (any name starting with \ppp{Potential} is interpreted as a separate component, and as always, parameter names and values are case-insensitive), and pass it as a command-line argument \texttt{accfile=\dots.ini}. The INI file may contain additional sections in more complicated cases:
{\setlength{\leftmargini}{6mm} \vspace{-2.5mm}
\begin{itemize} \setlength{\itemsep}{0pt} \setlength{\parskip}{0pt}
\item A section named \ppp{[Center]} defines the offset of the potential from the origin of the coordinate system used by the $N$-body simulation. It can be a fixed offset given by three parameters \ppp{x}, \ppp{y}, \ppp{z}, or a time-dependent offset specified by an auxiliary text file with $\ge 2$ lines and 4 columns: time, x, y, z. The name of this file should be given as a parameter \ppp{file=}\dots\ in this section of the INI file. The actual time-dependent offset will be interpolated by a cubic spline and linearly extrapolated beyond the range of times given in the file.
\item A section named \ppp{[Acceleration]} defines a spatially uniform acceleration field, which is supposed to be time-dependent and must be given in another auxiliary text file with the same 4-column structure, and the file name provided as a single parameter \ppp{file=}\dots\ in this section.
\item Finally, the potential itself may depend on time. In this case, it must be given by a sequence of potential expansion coefficients stored in separate files and an index file with a list of these files (two columns -- time and filename). The name of this index file should be provided as a parameter \ppp{file=}\dots\ in the section \ppp{[Time-dependent]} of the INI file. Alternatively, if there are no other sections in the INI file, it is not required, and the name of the index file itself should be provided as a command-line argument \texttt{accfile=\dots}
\end{itemize} }\vspace{-2.5mm}

Yet another possibility is to specify a pattern speed $\Omega$ for the potential (frequency of rotation of the potential figure about $z$ axis). In common with other \Nemo potentials, it is provided as a separate command-line argument \texttt{accpars=\dots} instead of the INI file.
...
For instance, this adds an extra potential in a \textsc{gyrfalcON}  simulation:\\
...
\texttt{mkplum out=sat.nemo nbody=10000 r_s=0.1 mass=0.1}\\
...
\texttt{snapshift sat.nemo sat_shift.nemo rshift=-1,0,0 vshift=0,-0.6,0}\\
Create an INI file (\texttt{pot.ini}) with the external potential:\\
\texttt{[Potential]\\type=Plummer\\mass=1\\scaleRadius=1}\\
...
\texttt{numpy.savetxt("accel.txt", numpy.column_stack((t, ax, ay, az)))}\\
And add the following lines to \texttt{pot.ini}:\\
\texttt{[Acceleration]\\file=accel.txt}\\
\texttt{[Center]\\file=center.txt}\\
...
In general, the non-inertial acceleration needs not correspond to the second derivative of the offset of the potential center -- one could imagine running the simulation in an inertial frame (hence zero acceleration), but forcing the external potential to dance around in some unpredictable way\dots And recall that one may add more than one external potential, each with its own trajectory!\\
\texttt{gyrfalcON ic.nemo out.nemo accname=agama,agama accfile=\char13 pot1.ini;pot2.ini\char13}
