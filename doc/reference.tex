...
\texttt{for trj in result1[:,1]: plt.plot((trj[:,0]**2 + trj[:,1]**2)**0.5, trj[:,2])}\\[1mm]
When the computation of deviation vectors is requested by the argument \texttt{der=True}, they are provided in the same format as the trajectory itself. There are six deviation vectors per orbit, so if \ppp{dtype}\texttt{=object}, they are stored as six \ttt{agama.Orbit} objects, otherwise as six arrays of shape $M_k\times$3 or 6. A matrix formed by column-stacking the deviation vectors at a given point on the orbit represents the Jacobian $\mathsf{J}(t)$ of mapping between the initial conditions $\boldsymbol w^{(0)}$ and the current point $\boldsymbol w(t)$: $J_{ij} = \partial w_i(t)/\partial w_j^{(0)}$:\\[1mm]
...
\texttt{linear_dif = numpy.einsum('ijk,k->ij', jac, delta)}\\
