...
\footnote{this restriction is due to the global interpreter lock (GIL) mechanism in \texttt{CPython}, precluding its simultaneous access from multiple threads.}. One may query or adjust the number of OpenMP threads by calling \texttt{agama.setNumThreads(N)}.
...
This class has only one method \texttt{__call__} (i.e., can be applied as a function), computing actions, and optionally angles and frequencies, for the given 6d position/velocity point or an $N\times6$ array of points:\\[1mm]
\texttt{act = af(points)}
