...
\item The density $\rho(\Phi(r)) = \int_{\Phi}^0 f\big(h(E)\big)\,4\pi\sqrt{2(E-\Phi)}\,dE$ is obtained from the DF \textit{in the given potential}, i.e., to update the density entering the Poisson equation, we use the current values of $f(h)$ and the previous potential (as long as these updates are frequent enough, there is no need to iteratively improve the solution by substituting the self-consistent potential back to the r.h.s.\ of this equation).
