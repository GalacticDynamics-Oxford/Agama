...
def test(kind):
    ...
    t_extr = spl.extrema()
    if len(t_extr) < 2 or numpy.any(t_extr[1:] <= t_extr[:-1]) or t_extr[0] != xval[0] or t_extr[-1] > xval[-1]:
...
if len(sys.argv) <= 1:
    ...
    def truefn(x, der=0):
        if der == 0:
            return numpy.sin(x * mul + off)
        if der == 1:
            return numpy.cos(x * mul + off) * mul
        if der == 2:
            return -numpy.sin(x * mul + off) * mul**2
        if der == 3:
            return -numpy.cos(x * mul + off) * mul**3
    ...
    print(
        "\033[1;32mApproximating a sine function with cubic and quintic splines.\033[0m\n"
    ...
    for spline, color in zip(splines, colors):
        ...
        def check(x, exempt):
            return (
                "" if spline == exempt or max(abs(x)) < 2e-14 else "\033[1;31m**\033[0m"
            )  # a fairly lousy tolerance!.. :(
