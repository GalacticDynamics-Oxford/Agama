...
def testHermite(pot, ic, time_in_periods, accuracy):
    ...
    t0 = time.time()
    orb_dop = agama.orbit(
        potential=pot, ic=ic, time=inttime, trajsize=0, dtype=float, accuracy=accuracy, method="dop853"
    )[1]
    t1 = time.time()
    orb_her = agama.orbit(
        potential=pot, ic=ic, time=inttime, trajsize=0, dtype=float, accuracy=accuracy, method="hermite"
    )[1]
    t2 = time.time()
    ...
    print(
        "Orbit integration with dop853,  accuracy %g: %6i steps, time=%.3f s, energy error=%s"
        % (accuracy, len(orb_dop), t1 - t0, check(errE_dop, accuracy**0.5))
    )
    print(
        "Orbit integration with hermite, accuracy %g: %6i steps, time=%.3f s, energy error=%s"
...
def testAccuracy(pot, ic, time_in_periods, accuracy, Omega=0, ax=None):
    ...
    def JacobiEnergy(orb):
        return (
            pot.potential(orb[:, 0:3])
            + 0.5 * numpy.sum(orb[:, 3:6] ** 2, axis=1)
    ...
    time_reg, traj_reg = agama.orbit(
        potential=pot,
        ic=ic,
        time=time,
        Omega=Omega,
        dtype=float,
        trajsize=abs(time_in_periods) * 100,
        accuracy=accuracy,
    )
    ...
    print(
        "regularly sampled orbit of length %i, energy error=%s" % (len(time_reg), check(numpy.max(errE_reg), errE_max))
    )
    ...
    print(
        "naturally sampled orbit of length %i, energy error at spline nodes=%s, at interpolated times=%s"
        % (len(orb_spl), check(numpy.max(errE_spl), errE_max), check(numpy.max(errE_spl_reg), 1.0 * accuracy**0.5))
    )
    print(
        "difference in position between interpolated and regularly sampled orbits=%s"
        % check(numpy.max(errX_spl_reg), 0.1 * accuracy**0.5)
    )
    print(
        "difference in position between Python and C++ versions of Orbit interpolator: %s"
...
def testDerivatives(pot, ic, time_in_periods, accuracy, Omega, ax=None):
    ...
    if ax is not None:
        ...
        ax.text(
            0.02,
            0.98,
            "solid - difference between nearby orbits\ndotted - deviation vectors",
            ha="left",
            va="top",
...
if __name__ == "__main__":
    ...
    ic = numpy.array([1, 0, 0.5, 0.0, 50.0, 0.0])
    ok = testAccuracy(pot, ic, -100, 1e-8, 50.0, ax[0])  # default accuracy
    ok &= testAccuracy(pot, ic, -100, 1e-10, 50.0, ax[1])  # higher  accuracy
    ok &= testDerivatives(pot, ic, 100, 1e-8, 50.0, ax[2])
    ok &= testHermite(pot, ic, 1000, 1e-4)
    ok &= testHermite(pot, ic, 1000, 1e-6)
    if plot:
        print(
            "The numerically computed orbit accumulates energy error (green curve) and "
        ...
        print(
            "In the right panel, we show the time evolution of the offset between "
