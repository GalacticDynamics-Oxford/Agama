...
class Component(object):
    ...
    def __init__(self, density=None, potential=None, **kwargs):
        ...
        for k, v in kwargs.items():
            self.__dict__[k.lower()] = v  # make parameters case-insensitive
...
class SelfConsistentModel(object):
    ...
    def _updatePotential(self):
        ...
        for component in self.components:
            ...
            if dens is not None:
                if component.disklike:
                    densityCyl.append(dens)
                else:
                    densitySph.append(dens)
            else:
...
if __name__ == "__main__":
    ...
    def test(Component, SelfConsistentModel):
        ...
        params_comp1 = dict(disklike=False, rminSph=0.01, rmaxSph=100.0, sizeRadialSph=21, lmaxAngularSph=0)
        params_comp2 = dict(disklike=False, rminSph=0.01, rmaxSph=10.0, sizeRadialSph=16, lmaxAngularSph=0)
        params_scm = dict(rminSph=0.005, rmaxSph=200, sizeRadialSph=25, lmaxAngularSph=0, verbose=False)

        potential_init = agama.Potential(
            type="spheroid", gamma=1, beta=3, alpha=1, mass=20.0, scaleRadius=5.0, outerCutoffRadius=40.0
        )
        df_comp1 = agama.DistributionFunction(type="quasispherical", density=potential_init, potential=potential_init)
        df_comp2 = agama.DistributionFunction(
            type="doublepowerlaw",
            norm=12.0,
            J0=1.0,
            coefJrIn=1.0,
