...
testCond(
    "isArray(dens.density([[1,0,0]]), (1,))"
)  # not equivalent: an array of 1 point is not the same as 1 point, but instead produces an 1d array of length 1
testCond(
    "isArray(dens.density( [1,0,0],[2,0,0] ), (2,))"
)  # more generally, N input points => 1d array of length N (here N=2)
testCond(
    "isArray(dens.density([[1,0,0],[2,0,0]]), (2,))"
)  # equivalent: an array of 2 points is the same as a tuple of 2 points
...
testCond(
    "dens.projectedDensity( 1,0, alpha=0,beta=0) > 0"
)  # can provide an input point as a pair of numbers, plus optional named arguments (angles)
testCond(
    "dens.projectedDensity([1,0],beta=0,gamma=0) > 0"
)  # first argument is an array, optional named arguments are angles
...
testCond(
    "dens.projectedDensity([[1,0]]).shape == (1,)"
)  # if the input is a 2d array (even with one row), output is a 1d array
testCond("dens.projectedDensity([[1,0],[2,0]],t=0).shape == (2,)")  # N input points (Nx2 array) => 1d array of length N
testCond(
    "dens.projectedDensity([[1,0],[2,0]],beta=0).shape == (2,)"
)  # one may specify a single value of angle for all points...
testCond(
    "dens.projectedDensity([[1,0],[2,0]],beta=(0,0),gamma=0,t=(0,0)).shape == (2,)"
)  # or one angle or time value per point (each angle can be a single point or an array independent of others)
testFail(
    "dens.projectedDensity([[1,0],[2,0]],beta=(0,0,0))"
)  # should fail when the size of the angles array does not match the number of points

testCond("isArray(dens.density([[1,2,3],[4,5,6]], t=1), (2,))")  # optional time argument as a single number
testCond(
    "isArray(dens.density([[1,2,3],[4,5,6]], t=[1,2]), (2,))"
)  # optional time argument as an array of the same length as points
...
testCond(
    "isTuple(pots.eval(1,0,0,pot=True,acc=True,der=True), 3)"
)  # evaluate three quantities at once: potential, acceleration and its derivatives
testCond("isArray(pots.eval(1,0,0,acc=True,t=0), (3,))")
testCond(
    "isArray(pots.eval([[1,2,3],[2,3,4]],der=True,t=[0,1]), (2,6))"
)  # one can specify times separately for each point
testCond(
    "isArray(pots.projectedEval(1,0,acc=True), (2,))"
)  # similar to projectedDensity, but produces two values for each input point consisting of two numbers
testCond(
    "isTuple(pots.projectedEval(1,1,pot=True,acc=True,der=True), 3)"
)  # evaluate three projected quantities at once: potential, acceleration and its derivatives
testCond(
    "isArray(pots.projectedEval([[1,0],[2,0],[3,2]],acc=True,beta=(0,0,0),gamma=0,t=(1,2,3)), (3,2))"
)  # one can specify angles and time separately for each input point or one for all (or not specify them at all)
...
testCond(
    "isArray(pots.Rperiapo( -1,1 ), (2,))"
)  # two numbers are a single point (E,L) => array of two values (Rperi,Rapo)
...
testCond(
    "isArray(pots.Rperiapo(numpy.random.random(size=(5,2))), (5,2))"
)  # a 2d array of (E,L) values with shape Nx2 => Nx2
testCond(
    "isArray(pots.Rperiapo(numpy.random.random(size=(5,6))), (5,2))"
)  # or a 2d array of (x,v) with shape Nx6 => Nx2
...
testCond(
    "isArray(actf([1,2,3,4,5,6]), (3,))"
)  # 1d array of length 6 is a single point (x,v) => output array of 3 actions
testFail("actf( 1,2,3,4,5,6 )")  # must be an array and not just 6 numbers
testCond(
    "isTuple(actf([1,2,3,4,5,6], frequencies=True), 2)"
)  # extra argument frequencies=True produces a tuple of two arrays (actions and frequencies)
testCond(
    "isTuple(actf([1,2,3,4,5,6], angles=True), 3)"
)  # extra argument angles=True implies frequencies=True and produces a tuple of length 3
testCond("isArray(actf([1,2,3,4,5,6], angles=True)[0], (3,))")  # with actions, angles and frequencies,
testCond(
    "isArray(actf([1,2,3,4,5,6], angles=True)[1], (3,))"
)  # each of them being an array of length 3 (for a single point)
testCond("isArray(actf([1,2,3,4,5,6], angles=True)[2], (3,))")
testCond(
    "isArray(actf([1,2,3,4,5,6], actions=False, frequencies=True), (3,))"
)  # when only frequencies but no actions are requested, the output is just an array of length 3 freqs (for a single point)
testCond(
    "isArray(actf(numpy.random.random(size=(5,6))), (5,3))"
)  # when the input has N points (Nx6 array), output is Nx3,
testCond(
    "isArray(actf(numpy.random.random(size=(5,6)), angles=True)[2], (5,3))"
)  # or three arrays of shape Nx3 if angles=True
testFail("agama.ActionFinder(pott)")  # should not work for non-axisymmetric potentials

# ActionMapper class methods
testCond(
    "isArray(actm([1,2,3,1,2,3]), (6,))"
)  # 1d array of length 6 (3 actions + 3 angles for a single point) => output 1d array of length 6 (x,v)
...
testCond(
    "isArray(agama.actions(pots, [1,2,3,4,5,6]), (3,))"
)  # input: potential, one or more points (x,v) => 3 actions per point
testCond(
    "isArray(agama.actions(potf, numpy.random.random(size=(42,6)), angles=True, fd=0.5)[2], (42,3))"
)  # optionally angles and frequencies of the same shape
...
testCond(
    "len(df2)==2 and df2[0](1,2,3) == df1(1,2,3)"
)  # df2 is a composite DF with two components, and the first one is df1
testCond(
    "len(df1([1,2,3], der=True))==2"
)  # DF derivatives w.r.t. actions are reported in the second element of the output tuple
testCond(
    "isArray(df1([1,2,3], der=True)[1], (3,)) and isArray(df1(numpy.random.random(size=(7,3)), der=True)[1], (7,3))"
)  # derivs array has the same shape as input actions
...
testCond(
    "isFloat(gmf1.moments([1,2,3], dens=True, vel=False,vel2=False))"
)  # only one output (density), and it is a scalar value
testCond(
    "isFloat(gmf1.moments( 1,2,3 , dens=True, vel=False,vel2=False))"
)  # a single input point may be given as three numbers, not an array of length three
testCond(
    "isArray(gmf1.moments([1,2,3], dens=False,vel=True, vel2=False),(3,))"
)  # one output (3 components of velocity)
testCond(
    "isArray(gms1.moments([1,2,3], dens=False,vel=False,vel2=True), (6,))"
)  # one output (elements of dispersion tensor) - 1d array of length 6
testCond(
    "isArray(gms1.moments([[1,2,3]],dens=True,vel=False,vel2=False),(1,))"
)  # input is an array of points with length 1, and so is the output
testCond(
    "isArray(gms2.moments([[1,2,3],[4,5,6],[7,8,9]], dens=True, vel=False, vel2=True)[1], (3,6))"
)  # N input points => dispersion tensor has shape Nx6
testCond(
    "isArray(gms2.moments([[1,2,3],[4,5,6],[7,8,9]], dens=False,vel=False, vel2=True, separate=True), (3,2,6))"
)  # DF with C components treated separately => NxCx6
testCond(
    "isArray(gms1.moments([[1,2,3],[4,5,6],[7,8,9]], dens=False,vel=False, vel2=True, separate=True), (3,1,6))"
)  # same here, but C=1 - output array is still 3d
testCond(
    "isArray(gms1.moments([1,2,3], dens=True, vel=False,vel2=False, separate=True), (1,))"
)  # single point, single output, but composite DF with C=1 => 1d array of length C
# check that separately computed moments of a composite DF (df2) agree with the moments of its 0th component (df1)
testCond(
    "numpy.isclose(gms1.moments([1,2,3],dens=1,vel=0,vel2=0), gms2.moments([1,2,3],dens=1,vel=0,vel2=0,separate=True)[0], 1e-4)"
)
# projected moments (input point(s) have only X,Y coordinates)
testCond(
    "isTuple(gms1.moments([1,0], dens=True, vel=True, vel2=True), 3)"
)  # same as before, but for the projected case  => output: a tuple of three numbers
testCond(
    "isArray(gms1.moments([[1,0],[2,0],[3,0],[4,0]], dens=True, vel=False, vel2=False), (4,))"
)  # input: array of N points, only one output array (density) with N elements
testCond(
    "isArray(gms2.moments([[1,0],[2,0],[3,0],[4,0]], dens=True, vel=False, vel2=False, separate=True), (4,2))"
)  # when separate=True, output arrays have one extra dimension of size C
# optional angle arguments
testCond(
    "isArray(gms1.moments([[1,2,3],[4,5,6]], beta=1, gamma=[1,2], dens=True, vel=False, vel2=False), (2,))"
)  # each of the orientation angles can be specified as single value for all points or one value per point
testFail("gms1.moments([1,2,3], beta=[1,2,3])")  # length of the angles array must match the number of input points

testCond(
    "isFloat(gms1.projectedDF([1,2,0,0,0,0,0,0]))"
)  # input: 8 numbers per point (x,y,vx,vy,vz,vxerr,vyerr,vzerr) => output: one number per point
testCond(
    "isArray(gms2.projectedDF(numpy.random.random(size=(4,8)), separate=True), (4,2))"
)  # when separate=True, output for N points and C components is a 2d array (NxC)

testCond("isTuple(gms1.vdf([1,2,3]), 3)")  # input: one point in 3d(xyz) => output 3 spline objects
testCond(
    "isArray(gms1.vdf([[1,2],[3,4]])[0], (2,))"
)  # input: two points in 2d(xy) => output a tuple of 3 arrays with splines
testCond(
    "isTuple(gms1.vdf([1,2,3], dens=True), 4)"
)  # if a flag dens=True is set, output is a tuple of length 4: 3 splines and density
testCond(
    "isArray(gms1.vdf([1,2,3])[0]([1,2,3,4,5]), (5,))"
)  # spline objects are functions that convert input arrays to output arrays of the same length
testCond(
    "isArray(gms2.vdf([[1,2,3],[4,5,6],[7,8,9]], separate=True)[0], (3,2))"
)  # when invoked for N points with separate=True, output arrays are NxC
...
testCond(
    "numpy.isclose(gms1.projectedDF([1,0,0,0,0,numpy.inf,numpy.inf,numpy.inf]), gms1.moments([1,0],vel2=False), 1e-3)"
)
...
xyv = numpy.column_stack(
    (vz * 0 + 1, vz * 0, vz * 0, vz * 0, vz, vz + numpy.inf, vz + numpy.inf, vz * 0)
)  # input points for projectedDF()
...
testCond(
    "isArray(gms2.moments(arr0x3, separate=False)[0], (0,)) and isArray(gms2.moments(arr0x3, separate=False)[1], (0,6))"
)
testCond(
    "isArray(gms2.moments(arr0x3, separate=True)[0], (0,2)) and isArray(gms2.moments(arr0x3, separate=True)[1], (0,2,6))"
)
