...
print(
    "Action mapping with spherical: %.3g s, rms error in posvel: %s, frequencies: %s"
    % (t2 - t1, checkLess(xv_sph - xv_iso, 1e-5), checkLess(omm_sph - omm_iso, 1e-7))
)
print(
    "Action mapping with torus, initial cost: %.3g s, repeated: %.3g s, rms error in posvel: %s, frequencies: %s"
    % (t3 - t2, t4 - t3, checkLess(xv_axi - xv_iso, 1e-3), checkLess(omm_axi - omm_iso, 1e-4))
)
print(
    "Action finding with isochrone: %.3g s, frequencies: %.3g s, actions+freq: %.3g s, act+ang+freq: %.3g s"
    % (t5a - t4, t5b - t5a, t5c - t5b, t5d - t5c)
)
print(
    "Isochrone (roundtrip), rms error in actions: %s, angles: %s, frequencies: %s"
    % (checkLess(act_iso - act, 1e-13), checkLess(ang_iso - ang, 1e-13), checkLess(omf_iso - omm_iso, 1e-13))
)
print(
    "Action finding with spherical (interpolated): %.3g s, frequencies: %.3g s, actions+freq: %.3g s, act+ang+freq: %.3g s"
    % (t6a - t5d, t6b - t6a, t6c - t6b, t6d - t6c)
)
print(
    "Spherical (interpol.), rms error in actions: %s, angles: %s, frequencies: %s"
    % (checkLess(act_spf - act, 1e-7), checkLess(ang_spf - ang, 2e-6), checkLess(omf_spf - omm_iso, 1e-6))
)
print(
    "Action finding with spherical (standalone):   %.3g s, frequencies: %.3g s, actions+freq: %.3g s, act+ang+freq: %.3g s"
    % (t7a - t6d, t7b - t7a, t7c - t7b, t7d - t7c)
)
print(
    "Spherical (st/alone),  rms error in actions: %s, angles: %s, frequencies: %s"
    % (checkLess(act_sps - act, 1e-10), checkLess(ang_sps - ang, 1e-11), checkLess(omf_sps - omm_iso, 1e-11))
)
print(
    "Action finding with axisym. Staeckel fudge:  %.3g s, frequencies: %.3g s, actions+freq: %.3g s, act+ang+freq: %.3g s"
    % (t8a - t7d, t8b - t8a, t8c - t8b, t8d - t8c)
)
print(
    "Axisym Staeckel fudge, rms error in actions: %s, angles: %s, frequencies: %s"
