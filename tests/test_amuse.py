...
class AgamaInterfaceTests(TestWithMPI):
    def test1(self):
        ...
        instance = Agama(
            type="Dehnen",
            mass=M | generic_unit_system.mass,
            scaleRadius=a | generic_unit_system.length,
        ...
        self.assertAlmostEqual(
            result.value_in(generic_unit_system.length**2 / generic_unit_system.time**2), -M / (r + a), places=14
        )
        ...
        self.assertAlmostEqual(
            result[0].value_in(generic_unit_system.length / generic_unit_system.time**2),
            -M / (r + a) ** 2 * x / r,
            places=14,
        )
        ...
        instance = Agama(
            ...
            type="Dehnen",
            mass=M * scaleM,
            scaleradius=a * scalea,
            channel_type="sockets",
            redirection="none",
        )
        result = instance.get_gravity_at_point(0.0 | generic_unit_system.length, x * scalea, y * scalea, z * scalea)
    ...
    def test2(self):
        ...
        instance = Agama(
            type="Multipole", particles=particles, symmetry="none", channel_type="sockets", redirection="none"
        )
        result = instance.get_potential_at_point(
            0.0 | generic_unit_system.length,
            0.0 | generic_unit_system.length,
        ...
        self.assertLess(
            abs(result / (-16.0 / 3 / 3.1416 | generic_unit_system.length**2 / generic_unit_system.time**2) - 1), 0.05
        )
