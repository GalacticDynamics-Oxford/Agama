...
def test(ra, dec, dist, pmra, pmdec, vlos):
    kwargs = dict(
        ra=ra * unit_angle,
        dec=dec * unit_angle,
        distance=dist * unit_dist,
        pm_ra_cosdec=pmra * unit_pm,
        pm_dec=pmdec * unit_pm,
        radial_velocity=vlos * unit_vlos,
    )
    try:
    ...
    diff_forward_inverse = numpy.amax(
        [abs(ra % (2 * numpy.pi) - a % (2 * numpy.pi)), abs(dec - d), abs(pmra - ma), abs(pmdec - md)]
    )
    ...
    diff_cartesian = numpy.amax(
        [
            abs(l % (2 * numpy.pi) - L % (2 * numpy.pi)),
            abs(b - B),
            abs(dist - D),
            abs(pml - ML),
            abs(pmb - MB),
    ...
    diff_galactocentric = numpy.amax(
        [
            abs(l % (2 * numpy.pi) - L % (2 * numpy.pi)),
            abs(b - B),
            abs(dist - D),
    ...
    kwargs = dict(
        galcen_distance=8.122 * unit_dist,
        z_sun=0.0208 * unit_dist,
        roll=-3.01077232808e-5 * unit.degree,
    ...
    print(
        (
            "Errors in conversion between ICRS<->Galactic: %.4g, Heliocentric Cartesian<->Sky: %.4g, "
            + "Galactocentric Cartesian<->Galactic: %.4g; Astropy ICRS<->Galactic: %.4g, "
            "Astropy Galactocentric<->Galactic: %.4g"
        )
        % (diff_forward_inverse, diff_cartesian, diff_galactocentric, diff_astropy_sky, diff_astropy_galactocentric)
    )
    return (
        (diff_forward_inverse < 1e-11)
        and (diff_cartesian < 1e-12)
        and (diff_galactocentric < 1e-12)
        and (diff_astropy_sky < 1e-12)
        and (diff_astropy_galactocentric < 1e-11)
    )
