...
def makeUserDensity(mass, radius):
    ...
    return lambda x: 3 / (4 * numpy.pi) * mass * radius**2 * (numpy.sum(x**2, axis=1) + radius**2) ** -2.5
...
print(
    "rho_appr(1)=%.8g, rho_app2(1)=%.8g  (true value=%.8g,  user value=%.8g)"
    % (
        pot_appr.density(1, 0, 0),
        pot_app2.density(1, 0, 0),
        pot_orig.density(1, 0, 0),
...
df_orig = agama.DistributionFunction(type="DoublePowerLaw", J0=J0, slopeIn=0, slopeOut=Beta, norm=2 * numpy.pi**3)
...
print(
    "original DF at r=1: density=%.8g, sigma_r=%.8g, sigma_t=%.8g"
...
print(
    "user-def DF at r=1: density=%.8g, sigma_r=%.8g, sigma_t=%.8g"
...
print("Total DF mass in a limited spatial region:  original=%.6g, user=%.6g" % (mass_gm_sf_orig, mass_gm_sf_user))
...
print("Total DF mass by sampling from this region: original=%.6g, user=%.6g" % (numpy.sum(m_orig), numpy.sum(m_user)))

if (
    abs(pot0_orig - pot0_appr) < 1e-6
    and abs(pot0_orig - pot0_app2) < 1e-6
    and abs(mass_orig - mass_user) < 1e-6
    and abs(mass_orig - mass_gm_orig) < 5e-3
    and abs(mass_user - mass_gm_user) < 5e-3
    and abs(dens_orig - dens_user) < 1e-6
    and abs(dens_user - dens_manual) < 1e-6
    and abs(mass_gm_sf_orig - numpy.sum(m_orig)) < 2e-2
    and all(abs(meanxv_orig - meanxv_user) < 0.005)
    and abs(m_orig[0] / m_user[0] - 1) < 0.001
    and
