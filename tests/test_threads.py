...
with agama.setNumThreads(2):
    ...
    targetl = agama.Target(
        type="LOSVD", degree=0, gridx=[0, 0.5, 1], gridv=[-1, 1], apertures=([[0, 0], [0.5, 1], [1, 0]],)
    )
    ...
    userden.density(
        numpy.random.normal(size=(10000, 3))
    )  # parallel loop in BatchFunction with vectorized calls to density
    ...
    agama.orbit(
        ic=numpy.random.normal(size=(4, 6)), potential=userpot, time=10, dtype=object
    )  # parallel loop over orbits
    ...
    scm.components.append(
        agama.Component(df=myfnc, disklike=False, lmaxAngularSph=0, sizeRadialSph=15, rminSph=0.01, rmaxSph=100)
    )
