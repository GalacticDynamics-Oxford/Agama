    ...
    def __init__(self, particles):
        ...
        if(len(good_elems)<len(rho_grid)):
            print("Invalid density encountered at r=" + \
                str(Radii[indices[numpy.where(numpy.logical_not(numpy.isfinite(rho_grid)))]]))
        ...
        print("Density slope: inner=%f, outer=%f" % (self.derrmin,self.derrmax))
...
def sampleZPosition(R, sph_model):
    ...
    while len(indices)>0:   # rejection sampling
        ...
        if(len(overflows)>0):
            print('Overflows: %i' % len(overflows))
