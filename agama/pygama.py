...
def getUnits():
    result = _getUnits()
    if not result:
        return result  # empty dict, no conversion needed
    import sys
...
def nonuniformGrid(nnodes, xmin, xmax=None):
    ...
    def fnc(A):
        if abs(A) < 1e-8:
            return nnodes - ratio + 0.5 * A * nnodes * (nnodes - 1)  # Taylor expansion
        else:
            return (_numpy.exp(A * (nnodes - 1)) - 1) / (_numpy.exp(A) - 1) - ratio

    A = scipy.optimize.brentq(fnc, _numpy.log(1 - 1.0 / ratio), _numpy.log(ratio) / (nnodes - 2))
...
fromICRStoGalactic = makeCelestialRotationMatrix(
    266.4049882865447 * _numpy.pi / 180, -28.93617776179147 * _numpy.pi / 180, 58.59866213832327 * _numpy.pi / 180
)
...
def transformCelestialCoords(
    rotationMatrix, lon, lat, pmlon=None, pmlat=None, sigmalon=None, sigmalat=None, sigmacorr=None
):
    """
    ...
    L = (
        _numpy.arctan2(M[1, 0] * x + M[1, 1] * y + M[1, 2] * z, M[0, 0] * x + M[0, 1] * y + M[0, 2] * z) + _numpy.pi
    ) % (2 * _numpy.pi) - _numpy.pi
    if pmlon is None:
        return L, B
    ...
    corrLB = (
        ((sigmalat**2 - sigmalon**2) * sinT * cosT + sigmacorr * sigmalon * sigmalat * (cosT**2 - sinT**2))
        / sigmaL
        / sigmaB
    )
...
def getGalacticFromGalactocentric(
    x, y, z, vx=None, vy=None, vz=None, galcen_distance=8.122, galcen_v_sun=(12.9, 245.6, 7.78), z_sun=0.0208
):
    """
...
def getGalactocentricFromGalactic(
    lon,
    lat,
    dist,
    pmlon=None,
...
def getViewingAngles(SXp, SYp, eta, Sx, Sy, Sz):
    ...
    if not (0 <= q and q <= v and v <= p and p <= u and u <= 1):
        raise ValueError(
            "Projected and assumed axis lengths are inconsistent (should be "
    ...
    if 1 - u < 1e-12:  # u=1 <=> SXp=Sx - assume an oblate axisymmetric system
        ...
        alpha = _numpy.arctan(
            ((1 - v * v) * (1 - u * u) / (p * p - v * v) / (u * u - p * p) * (p * p - q * q) / (1 - q * q)) ** 0.5
        )
    if p - v < 1e-12:
        psi = 0
        ...
        psi = _numpy.arctan(
            ((1 - v * v) / (1 - u * u) * (p * p - v * v) / (v * v - q * q) * (u * u - q * q) / (u * u - p * p)) ** 0.5
        )
...
def bsplineIntegrals(degree, grid, power=0):
    ...
    """
    return _numpy.sum(
        _numpy.vstack([_bsplines(degree, grid, x) * x**power * w for x, w in zip(*_bsplineGaussLegendre(grid))]), axis=0
    )
...
class GalpyPotential(_agama.Potential):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if (
            len(args) == 1
            and (isinstance(args[0], Potential) or isinstance(args[0], list))
            and (not kwargs or len(kwargs) == 1 and "symmetry" in kwargs)
        ):
            pot = args[0]
            ...
            if isinstance(pot, Potential):  # input is a native galpy potential
                ...
                if hasattr(pot, "_phiforce"):
                    self._phiforce = pot._phiforce
                if hasattr(pot, "_phi2deriv"):
                    self._phi2deriv = pot._phi2deriv
                if hasattr(pot, "_Rphideriv"):
                    self._Rphideriv = pot._Rphideriv
                if hasattr(pot, "_zphideriv"):
                    self._zphideriv = pot._zphideriv
                self._amp = pot._amp
                ...
                try:
                    ...
                    # if this works, define a vectorized wrapper
                    def wrapper(x):
                        return gp.evaluatePotentials(
                            pot, (x[:, 0] ** 2 + x[:, 1] ** 2) ** 0.5, x[:, 2], _numpy.arctan2(x[:, 1], x[:, 0])
                        )
                except Exception:
                    # define a non-vectorized wrapper
                    def wrapper(arr):
                        return _numpy.array(
                            [
                                gp.evaluatePotentials(pot, (x**2 + y**2) ** 0.5, z, _numpy.arctan2(y, x))
                                for x, y, z in arr
                            ]
                        )
                ...
                self._evaluate = lambda R, z, phi=0.0, t=0.0: gp.evaluatePotentials(pot, R, z, phi, t)
                self._dens = lambda R, z, phi=0.0, t=0.0: gp.evaluateDensities(pot, R, z, phi, t)
                self._Rforce = lambda R, z, phi=0.0, t=0.0: gp.evaluateRforces(pot, R, z, phi, t)
                self._zforce = lambda R, z, phi=0.0, t=0.0: gp.evaluatezforces(pot, R, z, phi, t)
                self._phiforce = lambda R, z, phi=0.0, t=0.0: gp.evaluatephiforces(pot, R, z, phi, t)
                self._R2deriv = lambda R, z, phi=0.0, t=0.0: gp.evaluateR2derivs(pot, R, z, phi, t)
                self._z2deriv = lambda R, z, phi=0.0, t=0.0: gp.evaluatez2derivs(pot, R, z, phi, t)
                self._phi2deriv = lambda R, z, phi=0.0, t=0.0: gp.evaluatephi2derivs(pot, R, z, phi, t)
                self._Rzderiv = lambda R, z, phi=0.0, t=0.0: gp.evaluateRzderivs(pot, R, z, phi, t)
                self._Rphideriv = lambda R, z, phi=0.0, t=0.0: gp.evaluateRphiderivs(pot, R, z, phi, t)
                self._zphideriv = lambda R, z, phi=0.0, t=0.0: gp.evaluatezphiderivs(pot, R, z, phi, t)
    ...
    def _phiforce(self, R, z, phi=0.0, t=0.0):
    ...
    def _Rphideriv(self, R, z, phi=0.0, t=0.0):
        ...
        acc, der = self.eval(coord, acc=True, der=True, t=t)
        return -(
            (der[1] - der[0]) * coord[1] * cphi
            + der[3] * (coord[0] * cphi - coord[1] * sphi)
            - acc[0] * sphi
            + acc[1] * cphi
        )

    def _zphideriv(self, R, z, phi=0.0, t=0.0):
...
class GalaPotential(_agama.Potential):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if (
            len(args) == 1
            and isinstance(args[0], PotentialBase)
            and (not kwargs or len(kwargs) == 1 and "symmetry" in kwargs)
        ):
            pot = args[0]
            sym = kwargs["symmetry"] if len(kwargs) == 1 else None
            try:
                PotentialBase.__init__(self, units=pot.units)
            except TypeError:
                PotentialBase.__init__(self, dict(), units=pot.units)
            ...
            for key in list(kwargs):
                if key == "units":
                    ...
                    del kwargs[key]
            try:
                PotentialBase.__init__(self, units=units)
            except TypeError:
                PotentialBase.__init__(self, dict(), units=units)
            _agama.Potential.__init__(self, *args, **kwargs)
            self._energy = lambda q, t=0.0: self.potential(q, t=t)
            self._density = lambda q, t=0.0: _agama.Potential.density(self, q, t=t)
            self._gradient = lambda q, t=0.0: -self.force(q, t=t)
            self._hessian = lambda q, t=0.0: -self.eval(q, der=True, t=t)[:, (0, 3, 5, 3, 1, 4, 5, 4, 2)].reshape(
                -1, 3, 3
            )
