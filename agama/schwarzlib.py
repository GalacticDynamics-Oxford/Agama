...
def makeDensityMGE(tab, distance, length_unit, beta):
    ...
    components = [
        getDensityParamsMGE(
            ...
            Sz=t[1] * arcsec2kpc / length_unit * (1 - (1 - t[2] ** 2) / _numpy.sin(beta) ** 2) ** 0.5,
        )  # third is smaller
        for t in tab
    ]
...
def surfaceDensityMGE(tab, xp, yp):
    ...
    """
    return _numpy.sum(
        [comp[0] * _numpy.exp(-0.5 * (xp**2 + (yp / comp[2]) ** 2) / comp[1] ** 2) for comp in tab], axis=0
    )
...
def makeDensityLogHalo(rcore, vcirc, rcutoff=None):
    ...
    return _agama.Density(
        dict(
            type="spheroid", alpha=2, beta=2, gamma=-2, densitynorm=rho0, scaleradius=rcore, outercutoffradius=rcutoff
        ),
        dict(
            type="spheroid",
            alpha=2,
...
def makeDensityNFWHalo(rscale, vcirc, rcutoff=None):
    ...
    return _agama.Density(
        type="spheroid", alpha=1, beta=3, gamma=1, densitynorm=rho0, scaleradius=rscale, outercutoffradius=rcutoff
    )
...
def makeMGE(particles, masses, beta, distance, plot=True):
    ...
    X, Y = _numpy.dot(_agama.makeRotationMatrix(alpha=0, beta=beta, gamma=0)[0:2], particles[:, :3].T)[
        0:2
    ]  # image-plane coords
    ...
    pixel = max(
        _numpy.percentile(
            (X * X + Y * Y) ** 0.5, 100.0 / len(particles)
        ),  # choose the pixel size to contain at least ~100 particles in the center,
        max(xmax, ymax) / 2000,
    )  # but don't make it too small, or else we run out of memory in constructing the histogram
    ...
    mge = mge_fit_sectors(
        ...
        fgal.eps,
        ngauss=15,
        qbounds=(_numpy.cos(beta) * 1.001, 1),
        sigmapsf=0.0,
        scale=pixel,
        linear=False,
        plot=plot,
    ...
    print(
        "Total mass in N-body snapshot: %.4g;  in the image: %.4g;  in MGE: %.4g"
        % (_numpy.sum(masses), _numpy.sum(hist), sum(mge.sol[0]))
    )
    ...
    if plot:
        ...
        mge_print_contours(
            hist, fgal.theta, xcenter, ycenter, mge.sol, binning=4, sigmapsf=1.0, magrange=12, scale=pixel
        )
...
def makeVoronoiBins(particles, gridx, gridy, nbins, alpha=0, beta=0, gamma=0, plot=True):
    ...
    bintags, xNode, yNode, xBar, yBar, sn, nPixels, scale = voronoi_2d_binning(
        ...
        hist,
        (hist + 0.1) ** 0.5,
        (1.0 * _numpy.sum(hist) / nbins) ** 0.5,
        plot=plot,
        quiet=False,
        pixelsize=min(min(gridx[1:] - gridx[:-1]), min(gridy[1:] - gridy[:-1])),
    )
    if plot:
        import matplotlib.pyplot as plt

        plt.gcf().axes[0].set_xlim(
            plt.gcf().axes[0].get_xlim()[::-1]
        )  # invert X axis as per the convention used in Agama
        print("*** Close all figures to continue ***")
        plt.show()
    ...
    for b in binTags:
        ...
        while edges < 1000:
            ...
            if direc == "?":
                ...
                vertices.append((xpixel * (cx + minx - 0.5) + xCoords[0], ypixel * (cy + miny - 0.5) + yCoords[0]))
...
def readApertures(filename):
    ...
    with open(filename) as dfile:
        polygons = [_numpy.array([float(a) for a in b.split()]).reshape(-1, 2) for b in dfile.read().split("\n\n")]
    result = _numpy.empty(len(polygons), dtype=object)
    result[:] = polygons
    return result
...
def makeGridForTargetLOSVD(polygons, psf, psfwingfrac=0.99, psfcorefrac=0.2, pixelmult=1.0, maxsize=100):
    ...
    print(
        (
            "Kinematic spaxel size is %.2f; %d apertures in the region [%.2f:%.2f, %.2f:%.2f]; "
...
class DensityDataset:
    ...
    def __init__(self, density, tolerance=0, alpha=0, beta=0, gamma=0, **target_params):
        ...
        self.cons_err[self.cons_err < 1e-12 * _numpy.max(abs(self.cons_val[1:]))] = 0.0
        ...
        print(
            "%s with %i constraints; total mass: %g; fraction of mass in %i density bins: %g"
    ...
    def projectedDensity(self, gridx, gridy):
        ...
        return self.density.projectedDensity(xy, alpha=self.alpha, beta=self.beta, gamma=self.gamma).reshape(
            len(gridx), len(gridy)
        )
...
class KinemDatasetVS:
        ...
        self.cons_err = _numpy.column_stack(
            (
                ...
                * (
                    ...
                    + vs_err[:, 1] ** 2 * (2 * vs_err[:, 1] ** 2 + 4 * vs_val[:, 1] ** 2)
                )
                ** 0.5,
            )
        ).reshape(-1)
    ...
    def getPenalty(self, model_losvd, Upsilon):
        ...
        s_mod = (
            model_losvd.reshape(self.num_aper, self.num_bsplines).dot(mod_bsint2) / a_mod * Upsilon - v_mod**2
        ) ** 0.5
...
class KinemDatasetGH:
    ...
    def __init__(self, density, ghm_val, ghm_err, tolerance=0, **target_params):
        ...
        self.cons_val = _numpy.column_stack((self.aperture_mass, ghm_val[:, 0:2] * 0, ghm_val[:, 2:])).reshape(-1)
        # constraint errors: translate errors in v0, sigma into errors in h_1,h_2
        self.cons_err = _numpy.column_stack(
            (self.aperture_mass_err, ghm_err[:, 0:2] / sigma[:, None] / 2**0.5, ghm_err[:, 2:])
        ).reshape(-1)
    ...
    def getOrbitMatrix(self, kinem_matrix, Upsilon):
        ...
        return _numpy.dstack(
            (
                ...
                _agama.ghMoments(
                    degree=self.mod_degree,
                    gridv=self.mod_gridv * Upsilon**0.5,
                    matrix=kinem_matrix,
                    ghorder=self.num_cons,
                    ghbasis=self.ghbasis,
    ...
    def getPenalty(self, model_losvd, Upsilon):
        ...
        ghm_mod = _agama.ghMoments(
            degree=self.mod_degree,
            gridv=self.mod_gridv * Upsilon**0.5,
            matrix=model_losvd * Upsilon**-0.5,
            ghorder=self.num_cons,
    ...
    def getGHMoments(self, LOSVD=None, Upsilon=None):
        ...
        if LOSVD is None:
            ...
            return _agama.ghMoments(
                matrix=LOSVD * Upsilon**-0.5, gridv=self.mod_gridv * Upsilon**0.5, degree=self.mod_degree, ghorder=6
            )[:, (1, 2, 6, 7, 8, 9)]
        ...
        for n in range(self.num_aper):
            boots = _agama.ghInterp(
                ...
                _numpy.vstack(
                    (
                        _numpy.ones(nboot),
                        _numpy.zeros((2, nboot)),  # coefs: h0=1, h1=h2=0, remaining are perturbed by noise
                        self.ghm_val[n, 2:][:, None]
                        + _numpy.random.normal(size=(self.num_cons - 2, nboot)) * self.ghm_err[n, 2:][:, None],
                    )
                ),
                gridv,
            )
...
class KinemDatasetHist:
        ...
        return _numpy.column_stack(
            (
                ...
                * _numpy.linalg.solve(
                    ...
                    _agama.bsplineMatrix(
                        self.obs_degree, self.obs_gridv, self.mod_degree, self.mod_gridv * Upsilon**0.5
                    ),
                ).T,
            )
        )
    ...
    def getOrbitMatrix(self, kinem_matrix, Upsilon):
        ...
        num_orbits = len(kinem_matrix)
        return (
            kinem_matrix.reshape(num_orbits, self.num_aper, self.num_bsplines)
    ...
    def getPenalty(self, model_losvd, Upsilon):
        ...
        Returns: the error-weighted penalty (chi^2) separately for each bin in the histogram
          (including the aperture mass as the 0th element) summed over all apertures.
        """
        cons_mod = model_losvd.reshape(self.num_aper, self.num_bsplines).dot(self._getConvMatrix(Upsilon)).reshape(-1)
        use = self.cons_err != 0
    ...
    def getGHMoments(self, LOSVD=None, Upsilon=None):
        ...
        ghorder = 6
        ind = tuple(
            [1, 2] + range(6, ghorder + 4)
        )  # indices of columns containing v,sigma,h3...hM in the matrix returned by ghMoments
        if LOSVD is None:
            ghm_val, ghm_err = ghMomentsErrors(
                degree=self.obs_degree, gridv=self.obs_gridv, values=self.obs_val, errors=self.obs_err, ghorder=ghorder
            )
            return ghm_val[:, ind], ghm_err[:, ind]
        else:
            return _agama.ghMoments(
                matrix=LOSVD * Upsilon**-0.5,
                gridv=self.mod_gridv * Upsilon**0.5,
                degree=self.mod_degree,
                ghorder=ghorder,
            )[:, ind]
...
def runModel(
    ...
    try:
        len(datasets)  # ok if it is a tuple/list
    except:
        datasets = (datasets,)  # if not, make it a tuple
    if len(datasets) == 0:
    ...
    matrices = _agama.orbit(
        potential=potential, ic=ic, time=inttime, Omega=Omega, targets=[d.target for d in datasets], dtype=object
    )
    ...
    def solve(Upsilon):
        ...
        pen_reg = 2.0 * regul * _numpy.ones(numOrbits) * numOrbits / totalMass**2
        ...
        for ds, pen, sup in zip(datasets, penalties, superpositions):
            print(
                "Penalty for %i %s constraints: %s %g" % (len(ds.cons_val), str(ds.target), str(pen), _numpy.sum(pen))
            )
        ...
        print(
            "Penalty for regularization: %7.2f;  entropy: %.3g;  # of useful orbits: %i / %i"
    ...
    if nbody:
        ...
        if status:
            _agama.writeSnapshot(
                filePrefix + "_Y%.3f.nbody" % this.Upsilon,
                (
                    _numpy.hstack((particles[0][:, 0:3], particles[0][:, 3:6] * this.Upsilon**0.5)),
                    particles[1] * this.Upsilon,
                ),
                nbodyFormat,
            )
        else:
...
def runPlot(
    datasets,  # list of [kinematic] datasets to be plotted
    aval=[],
    bval=[],
    chi2=[],
    filenames=[],  # data for the chi2(a,b) plot, and the list of associated LOSVD files
    interp="linear",
    alabel="",
    blabel="",  # parameters  for the chi2(a,b) plot
    alim=None,
    blim=None,
    deltaChi2lim=100,  # axes ranges for the chi2(a,b) plot
    xlim=None,
    ylim=None,  # ranges for x,y coordinates on kinematic maps
    v0lim=None,
    sigmalim=None,
    hlim=(-0.1, 0.1),  # ranges for color axes on kinematic maps for v0,sigma,h3..h6
    v0err=None,
    sigmaerr=None,
    herr=0.1,  # ranges for color axes on maps of errors in  v0,sigma,h3..h6
    ...
    def loadModel(modelIndex):
        ...
        try:
            ...
            try:
                archive = _numpy.load(archiveFilename, allow_pickle=True, encoding="latin1")
            except TypeError:
                archive = _numpy.load(archiveFilename)  # older version of numpy
            ...
            for ds in datasets:
                if hasattr(ds, "mod_gridv"):
                    ...
                    if hasattr(ds, "getGHMoments"):
                        ...
                        ghm_mod.append(
                            _agama.ghMoments(
                                matrix=LOSVD[indexKinemDataset][archiveIndex] * Upsilon**-0.5,
                                gridv=ds.mod_gridv * Upsilon**0.5,
                                degree=ds.mod_degree,
                                ghorder=6,
                            )[:, (1, 2, 6, 7, 8, 9)]
                        )
                    indexKinemDataset += 1
    ...
    def plotWeights(archive, archiveIndex):
        ...
        if this.axo is None:  # create axes on the first call
            ...
            axa.imshow(
                _numpy.linspace(0, 1, 256).reshape(-1, 1),
                extent=[0, 1, -1, 1],
                origin="lower",
        ...
        this.sco = this.axo.scatter(
            R[use],
            L[use],
            s=2 * weights[use] * len(weights),
            marker="o",
            picker=3,
            c=I[use],
            cmap="mist",
    ...
    def plotLOSVD():
        ...
        if hasattr(this, "los_mod"):
            this.losvdplot.plot(
                plot_gridv, _agama.bsplineInterp(this.mod_degree, this.mod_gridv, this.los_mod[ind], plot_gridv), "k"
            )[0].set_dashes([5, 2])
        this.losvdplot.set_xlim(min(plot_gridv), max(plot_gridv))
        this.losvdplot.set_yticklabels([])
        this.losvdplot.set_xlabel("v")
        ...
        text = "Aperture #%i centered at x=%.3f, y=%.3f: " % (
            ind,
            _numpy.mean(apertures[ind][:, 0]),
            _numpy.mean(apertures[ind][:, 1]),
        )
        for i in range(6):
            ...
            if hasattr(this, "ghm_mod"):
                ...
                if err < -1.0:
                    text += "[\033[1;31m %.3f \033[0m] " % this.ghm_mod[ind, i]
                elif err > 1.0:
                    text += "[\033[1;34m %.3f \033[0m] " % this.ghm_mod[ind, i]
                else:
                    text += "[ %.3f ] " % this.ghm_mod[ind, i]
        print(text)
    ...
    def plotOrbit(indOrbit):
        ...
        print(
            "Selected orbit #%i" % indSel + (" from a list of %i orbits" % len(indOrbit) if len(indOrbit) > 1 else "")
        )
    ...
    for d in datasets:  # loop over kinematic datasets only
        if hasattr(d, "getGHMoments"):
            ...
            for i, a in enumerate(d.target_params["apertures"]):
                apertures.append(
                    _numpy.column_stack((a[:, 0] * cosg - a[:, 1] * sing, a[:, 1] * cosg + a[:, 0] * sing))
                )
    ...
    if xlim is None:
        xlim = (min([_numpy.amin(p[:, 0]) for p in apertures]), max([_numpy.amax(p[:, 0]) for p in apertures]))
    if ylim is None:
        ylim = (min([_numpy.amin(p[:, 1]) for p in apertures]), max([_numpy.amax(p[:, 1]) for p in apertures]))
    if vlim is None:
        vlim = (min(ghm_val[:, 0] - ghm_val[:, 1] * 3.0), max(ghm_val[:, 0] + ghm_val[:, 1] * 3.0))
    ...
    if v0lim is None:
        v0lim = max(abs(ghm_val[:, 0]) + ghm_err[:, 0]) * _numpy.array([-1, 1])  # symmetric range
    if sigmalim is None:
        sigmalim = (min(ghm_val[:, 1] - ghm_err[:, 1]), max(ghm_val[:, 1] + ghm_err[:, 1]))
    if v0err is None:
        v0err = max(ghm_err[:, 0])
    if sigmaerr is None:
        sigmaerr = max(ghm_err[:, 1])
    panel_params = [
    ...
    for param in panel_params:
        ...
        ax.contour(
            plot_gridx,
            plot_gridy,
            projectedDensityMag.T,
            levels=_numpy.linspace(0, 10, 11),
            cmap="Greys_r",
            vmin=0,
            vmax=11,
        )
    ...
    if len(aval) > 0:
        ...
        this.modellabel = ax.text(
            0.01, 0.01, "", color="r", ha="left", va="bottom", transform=ax.transAxes, fontsize=10
        )
        ax.text(
            0.5,
            0.99,
            r"$\mathrm{min}\,\chi^2=%.2f$" % min(chi2),
            color="r",
            ha="center",
            va="top",
        ...
        if alim is None:
            alim = (min(aval), max(aval))
        if blim is None:
            blim = (min(bval), max(bval))
        anorm = alim[1] - alim[0]
        bnorm = blim[1] - blim[0]
        ...
        try:
            ...
            if interp == "linear" or interp == "cubic":
                c = scipy.interpolate.griddata(
                    (aval / anorm, bval / bnorm), chi2 - min(chi2), (a / anorm, b / bnorm), method=interp
                )
            elif interp == "rbf":
                c = scipy.interpolate.Rbf(aval / anorm, bval / bnorm, chi2 - min(chi2), function="thin_plate")(
                    a / anorm, b / bnorm
                )
            else:
            ...
            cntr = _numpy.array(
                [2.30, 6.18, 11.83] + [x**2 + _numpy.log(x**2 * _numpy.pi / 2) + 2 * x**-2 for x in range(4, 33)]
            )
            ax.contourf(a, b, c, cntr, cmap="hell_r", vmin=0, vmax=deltaChi2lim, alpha=0.75)
            ax.clabel(
                ax.contour(a, b, c, cntr, cmap="Blues_r", vmin=0, vmax=deltaChi2lim), fmt="%.0f", fontsize=10, inline=1
            )
            ...
            ax.text(
                1.0, 0.95, r"$\Delta\chi^2$", color="r", clip_on=False, ha="left", va="center", transform=ax.transAxes
            )
        except:
            traceback.print_exc()
    else:
