...
class AgamaInterface(CodeInterface, LiteratureReferencesMixIn, GravitationalDynamicsInterface, GravityFieldInterface):
    ...
    def set_time_step():
        function = LegacyFunctionSpecification()
        function.addParameter("time_step", dtype="float64", direction=function.IN, description="Set the episode length")
        function.result_type = "int32"
        return function
    ...
    def set_num_threads():
        ...
        function.addParameter(
            "num_threads", dtype="int32", direction=function.IN, description="Number of OpenMP threads"
        )
        function.result_type = "int32"
        return function
    ...
    def get_gravitating_mass():
        function = LegacyFunctionSpecification()
        function.addParameter(
            "index_of_the_particle", dtype="int32", direction=function.IN, description="Index of the particle"
        )
        function.addParameter(
            "gravitating_mass",
            dtype="float64",
...
class Agama(GravitationalDynamics, GravityFieldCode):
    def __init__(self, unit_converter=None, number_of_workers=None, **options):
        ...
        for k, v in options.items():
            ...
            if param_name.lower() in dimensional_params.keys():
                ...
                if param_name.lower() == "mbh":
                    black_hole_provided = True
    ...
    def define_methods(self, handler):
        ...
        handler.add_method("set_time_step", (nbody_system.time,), (handler.ERROR_CODE,))
        handler.add_method(
            "get_gravity_at_point", (nbody_system.length,) * 4, (nbody_system.acceleration,) * 3 + (handler.ERROR_CODE,)
        )
        handler.add_method(
            "get_potential_at_point", (nbody_system.length,) * 4, (nbody_system.potential, handler.ERROR_CODE)
        )
        handler.add_method("set_gravitating_mass", (handler.NO_UNIT, nbody_system.mass), (handler.ERROR_CODE,))
        handler.add_method("get_gravitating_mass", (handler.NO_UNIT,), (nbody_system.mass, handler.ERROR_CODE))
