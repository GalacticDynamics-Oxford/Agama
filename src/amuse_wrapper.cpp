...
particles::ParticleArray<coord::PosCar> bodies;  // array of particles that are used to compute potential
}
...
int set_particles(double x[], double y[], double z[], double m[], int n)
{
    ...
    bodies.data.resize(n);
    for(int i=0; i<n; i++) {
        bodies.data[i].first.x=x[i];
        bodies.data[i].first.y=y[i];
        bodies.data[i].first.z=z[i];
        bodies.data[i].second =m[i];
    }
    return 0;
}
...
int initialize_code(int nparams, char** params)
{
    ...
    try{
        ...
        if(!bodies.data.empty())
            pot = potential::createPotential(args, bodies, units::ExternalUnits());
        else
            pot = potential::createPotential(args);
    }
    catch(std::exception& e) {
        // print the message directly to the terminal, otherwise it gets swallowed somewhere inside amuse...
        std::ofstream("/dev/tty") << e.what() << '\n';
        return -1;
    }
    return 0;
}
