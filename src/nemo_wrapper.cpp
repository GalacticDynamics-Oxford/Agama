...
static double cosphi = 1, sinphi = 0;
...
inline void getAcc(int nbody, const int* active, const void* _pos, void* _pot, void* _acc)
{
    if(!mypot)
...
inline void getAcc(int nbody, const int* active, const void* pos, void* pot, void* acc, int flag)
{
    switch(flag & 3) {
        case 0: getAcc<NumT, NDIM, 0, 0>(nbody, active, pos, pot, acc); break;
        case 1: getAcc<NumT, NDIM, 1, 0>(nbody, active, pos, pot, acc); break;
        case 2: getAcc<NumT, NDIM, 0, 1>(nbody, active, pos, pot, acc); break;
        case 3: getAcc<NumT, NDIM, 1, 1>(nbody, active, pos, pot, acc); break;
        default: ;
    }
}
    ...
    char numtype)       /* input:  type: 'f' or 'd'                */
{
    ...
    if(numtype=='f') {
        if(NDIM==2)
            getAcc<float, 2>(nbody, active, pos, pot, acc, flag);
        else if(NDIM==3)
            getAcc<float, 3>(nbody, active, pos, pot, acc, flag);
        else throw std::runtime_error("Invalid NDIM");
    } else if(numtype=='d') {
        if(NDIM==2)
            getAcc<double, 2>(nbody, active, pos, pot, acc, flag);
        else if(NDIM==3)
            getAcc<double, 3>(nbody, active, pos, pot, acc, flag);
        else throw std::runtime_error("Invalid NDIM");
    ...
    const double*time)
{
    math::sincos(Omega * (*time), cosphi, sinphi);
    if(*ndim==2)
        getAcc<double, 2, 0, 0>(1, NULL, pos, pot, acc);
    else if(*ndim==3)
        getAcc<double, 3, 0, 0>(1, NULL, pos, pot, acc);
    else throw std::runtime_error("Invalid NDIM");
}
    ...
    const float *time)
{
    math::sincos(Omega * (*time), cosphi, sinphi);
    if(*ndim==2)
        getAcc<float, 2, 0, 0>(1, NULL, pos, pot, acc);
    else if(*ndim==3)
        getAcc<float, 3, 0, 0>(1, NULL, pos, pot, acc);
    else throw std::runtime_error("Invalid NDIM");
}
