...
template<typename CoordT> struct PointMassArray {
    ...
    inline ElemType& at(unsigned int index) { return data.at(index); }             // convenience shorthand
    inline const ElemType& at(unsigned int index) const { return data.at(index); } // convenience shorthand
    inline ElemType& operator[](unsigned int index) { return data[index]; }        // convenience shorthand
    inline const ElemType& operator[](unsigned int index) const { return data[index]; } // another shorthand
    inline unsigned int size() const { return data.size(); }  // convenience shorthand
